...
public class DreadOverlay {
    public static final HudRenderCallback OVERLAY = DreadOverlay::drawOverlay;
    private static final Minecraft minecraft = Minecraft.getInstance();

    public static void drawOverlay(GuiGraphics guiGraphics, float partialTick) {
        if (minecraft.player != null) {
            Player player = minecraft.player;
            Wight wight = Wight.findWight(player);
            if (wight != null) {
                ResourceLocation overlay;
                int frame = (int) (partialTick % 16);
                ...
                renderOverlay(overlay, alpha, guiGraphics.guiWidth(), guiGraphics.guiHeight());
            }
        }
    }
