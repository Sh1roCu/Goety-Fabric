...
public class DragonFlameParticle extends ReversibleParticle {
    ...
    public void tick() {
        ...
        if (!this.removed) {
            ...
            ParticleOptions ground = ModParticleTypes.SMALL_DRAGON_FLAME_GROUND;
            if (this.isNecro) {
                ground = ModParticleTypes.NECRO_DRAGON_FLAME_GROUND;
            }
            if (this.isBig) {
                if (this.random.nextFloat() <= 0.25F) {
                    this.level.addParticle(ModParticleTypes.DRAGON_FLAME_DROP, this.x, this.y, this.z, this.xd, this.yd, this.zd);
                }
                ground = ModParticleTypes.BIG_SOUL_FIRE_GROUND;
            }
            if (this.onGround) {
