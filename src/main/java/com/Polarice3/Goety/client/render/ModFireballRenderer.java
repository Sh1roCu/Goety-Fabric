...
import cn.sh1rocu.goety.mixin.accessor.EntityRenderDispatcherAccessor;
...
public class ModFireballRenderer<T extends Entity & ItemSupplier> extends ThrownItemRenderer<T> {
    ...
    public void render(T p_116085_, float p_116086_, float p_116087_, PoseStack p_116088_, MultiBufferSource p_116089_, int p_116090_) {
        ...
        if (p_116085_.getType() == ModEntityType.MOD_FIREBALL) {
            entityType = EntityType.SMALL_FIREBALL;
        } else if (p_116085_.getType() == ModEntityType.LAVABALL) {
            entityType = EntityType.FIREBALL;
        }
        try {
            EntityRenderer<T> entityRenderer = (EntityRenderer<T>) ((EntityRenderDispatcherAccessor) this.entityRenderDispatcher).goety$getRenderers().get(entityType);
