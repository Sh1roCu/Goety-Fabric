...
public class WearRenderer implements TrinketRenderer {

    private final ResourceLocation texture;
    private final Supplier<HumanoidModel<LivingEntity>> modelSupplier;
    @Nullable
    private HumanoidModel<LivingEntity> model = null;

    public WearRenderer(ResourceLocation texture, Supplier<HumanoidModel<LivingEntity>> modelSupplier) {
        this.texture = texture;
        this.modelSupplier = modelSupplier;
    }

    private ResourceLocation getTexture(LivingEntity livingEntity) {
        if (Objects.equals(this.texture, TrinketsRenderer.render("necro_cape.png"))) {
            if (ItemConfig.NecroCapeChangeTexture.get()) {
                if (CuriosFinder.hasFrostRobes(livingEntity)) {
                    return TrinketsRenderer.render("necro_cape_crypt.png");
                } else if (CuriosFinder.hasAbyssRobes(livingEntity)) {
                    return TrinketsRenderer.render("necro_cape_abyss.png");
                } else if (CuriosFinder.hasNetherRobe(livingEntity)) {
                    return TrinketsRenderer.render("necro_cape_nether.png");
                } else if (CuriosFinder.hasWildRobe(livingEntity)) {
                    return TrinketsRenderer.render("necro_cape_wild.png");
                }
            }
        } else if (Objects.equals(this.texture, TrinketsRenderer.render("unholy_hat.png"))) {
            if (MobUtil.healthIsHalved(livingEntity)) {
                return TrinketsRenderer.render("unholy_hat_red.png");
            }
    ...
    public static WearRenderer getRenderer(ItemStack stack) {
        if (!stack.isEmpty()) {
            return (WearRenderer) TrinketRendererRegistry.getRenderer(stack.getItem()).orElse(null);
        }
        return null;
    }
    ...
    public static boolean capedRobe(ItemStack stack) {
        return stack.getItem() == ModItems.GRAND_ROBE || stack.getItem() == ModItems.FROST_ROBE || stack.getItem() == ModItems.FROST_ROBE_CRYO || stack.getItem() == ModItems.WIND_ROBE || stack.getItem() == ModItems.STORM_ROBE;
    }
            ...
            if (livingEntity instanceof AbstractClientPlayer p_116618_) {
                if (ItemConfig.RobeCape.get()
                    ...
                    if (!(itemstack.getItem() instanceof ElytraItem)) {
                        ...
                        double d0 = Mth.lerp(partialTicks, p_116618_.xCloakO, p_116618_.xCloak) - Mth.lerp(partialTicks, p_116618_.xo, p_116618_.getX());
                        double d1 = Mth.lerp(partialTicks, p_116618_.yCloakO, p_116618_.yCloak) - Mth.lerp(partialTicks, p_116618_.yo, p_116618_.getY());
                        double d2 = Mth.lerp(partialTicks, p_116618_.zCloakO, p_116618_.zCloak) - Mth.lerp(partialTicks, p_116618_.zo, p_116618_.getZ());
    ...
    private void render(LivingEntity livingEntity, ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer, int light) {
        ...
        if (stack.getItem() instanceof UnholyHatItem) {
            ...
            VertexConsumer vertexBuilder2 = buffer.getBuffer(ModRenderType.wraith(TrinketsRenderer.render(glow)));
