            ...
            if (level > 0) {
                if (entity.isHeated() && !Minecraft.getInstance().isPaused()) {
                    ...
                    if (fluidHeight > 0) {
                        ...
                        world.addParticle(ModParticleTypes.BREW_BUBBLE, pos.getX() + 0.5 + Mth.nextDouble(world.random, -width, width), pos.getY() + fluidHeight, pos.getZ() + 0.5 + Mth.nextDouble(world.random, -width, width), rgb[0], rgb[1], rgb[2]);
                    }
                }
            }
        }
