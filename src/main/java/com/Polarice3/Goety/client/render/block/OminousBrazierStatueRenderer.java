...
public class OminousBrazierStatueRenderer implements BlockEntityRenderer<OminousBrazierStatueBlockEntity> {
    ...
    @Override
    public boolean shouldRenderOffScreen(OminousBrazierStatueBlockEntity blockEntity) {
        return true;
    }

    @Override
    public boolean shouldRender(OminousBrazierStatueBlockEntity blockEntity, Vec3 vec3) {
        return Vec3.atCenterOf(blockEntity.getBlockPos()).multiply(1.0D, 0.0D, 1.0D).closerThan(vec3.multiply(1.0D, 0.0D, 1.0D), (double)this.getViewDistance());
    }
}