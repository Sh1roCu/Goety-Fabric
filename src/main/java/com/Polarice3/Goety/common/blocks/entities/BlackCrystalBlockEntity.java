...
public class BlackCrystalBlockEntity extends OwnedBlockEntity implements IEnchantedBlock {
    ...
    public BlackCrystalBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.BLACK_CRYSTAL, p_155229_, p_155230_);
    }
    ...
    public CompoundTag writeNetwork(CompoundTag tag) {
        this.saveEnchants(tag, ModBlocks.BLACK_CRYSTAL.asItem());
    ...
    public void tick() {
        if (this.level != null) {
            ...
            if (this.level instanceof ServerLevel serverLevel) {
                int radius = 8 + this.enchantments.getOrDefault(ModEnchantments.RADIUS, 0);
                if (this.target == null) {
                    ...
                    if (!this.target.isAlive() || this.target.getHealth() < this.target.getMaxHealth() * 0.25F || this.target.distanceToSqr(this.getBlockPos().getCenter()) > Mth.square(radius)) {
                        ...
                        if (this.attackTick % 20 == 0) {
                            ...
                            if (this.target.hurt(damageSource, this.target.getMaxHealth() * 0.1F)) {
                                ...
                                this.level.playSound(null, this.worldPosition, ModSounds.SOUL_EAT, SoundSource.BLOCKS, 1.0F, 1.0F);
                                if (this.getTrueOwner() instanceof Player player) {
                                    int enchantment = this.enchantments.getOrDefault(ModEnchantments.SOUL_EATER, 0);
