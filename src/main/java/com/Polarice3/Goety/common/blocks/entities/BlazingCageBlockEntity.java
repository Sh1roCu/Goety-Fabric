...
public class BlazingCageBlockEntity extends TrainingBlockEntity {
    ...
    public BlazingCageBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.BLAZING_CAGE, p_155229_, p_155230_);
    }
        ...
        if (blockEntity.isTraining()) {
            if (blockEntity.trainTime != blockEntity.getMaxTrainTime()) {
                ...
                if (this.trainTime == 20) {
                    level.playSound(null, blockPos, ModSounds.BLAZING_CAGE_TRAIN, SoundSource.BLOCKS, 1.0F, 1.0F);
                }
            } else {
                if (level instanceof ServerLevel serverLevel) {
                    double d0 = (double) blockPos.getX() + level.random.nextDouble();
    ...
    public void setVariant(ItemStack itemStack, Level level, BlockPos blockPos) {
        if (level instanceof ServerLevel) {
            if (this.getTrainMob() != ModEntityType.BLAZE_SERVANT) {
                this.setEntityType(ModEntityType.BLAZE_SERVANT);
                this.markUpdated();
            }
        }
    }
    ...
    public void startTraining(int amount, ItemStack itemStack) {
        ...
        if (this.level != null) {
            this.level.playSound(null, this.getBlockPos(), ModSounds.BLAZING_CAGE_START, SoundSource.BLOCKS, 1.0F, 1.0F);
        }
    }
    ...
    public void playSpawnSound() {
        if (this.level != null) {
            this.level.playSound(null, this.getBlockPos(), ModSounds.SUMMON_SPELL_FIERY, SoundSource.BLOCKS, 1.0F, 1.0F);
        }
    }
