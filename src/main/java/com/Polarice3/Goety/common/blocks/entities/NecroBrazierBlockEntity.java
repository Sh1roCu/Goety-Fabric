...
public class NecroBrazierBlockEntity extends ModBlockEntity implements Clearable {
    ...
    public NecroBrazierBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.NECRO_BRAZIER, p_155229_, p_155230_);
    }
    ...
    public boolean activate(Level world) {
        if (this.getRecipe() == null) {
            BrazierRecipe brazierRecipe = world.getRecipeManager().getAllRecipesFor(ModRecipeSerializer.BRAZIER_TYPE).stream().filter(
                ...
                if (!this.level.isClientSide) {
                    ...
                    if (this.level.random.nextFloat() < 0.3F) {
                        if (this.level.random.nextFloat() < 0.17F) {
                            ModNetwork.sendToALL(serverWorld.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(this.worldPosition, SoundEvents.FURNACE_FIRE_CRACKLE, 0.5F + this.level.random.nextFloat(), this.level.random.nextFloat() * 0.7F + 0.3F));
                        }
                    }
                }
                if (!this.candlestickBlockEntityList.isEmpty()) {
                    ...
                    if (!this.level.isClientSide) {
                        ...
                        if (this.activate(this.level)) {
                            if (recipe != null) {
                                for (int p = 0; p < 2; ++p) {
                                    serverWorld.sendParticles(ModParticleTypes.SMALL_NECRO_FIRE, d0, this.worldPosition.getY() + 0.5F, d2, 1, 0, 0, 0, 0);
                                    serverWorld.sendParticles(ParticleTypes.SMOKE, d0, d1, d2, 0, 0.0D, 5.0E-4D, 0.0D, 0.5F);
                                    serverWorld.sendParticles(ModParticleTypes.NECRO_EFFECT, d0, d1, d2, 1, 0.0F, 0.0F, 0.0F, 0.0F);
                                }
                                ...
                                if (this.currentTime == 1) {
                                    ModNetwork.sendToALL(serverWorld.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(this.worldPosition, SoundEvents.BLAZE_AMBIENT, 1.0F, this.level.random.nextFloat() * 0.1F + 0.9F));
                                }
                                if (this.level.getGameTime() % 20 == 0) {
                                    ModNetwork.sendToALL(serverWorld.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(this.worldPosition, SoundEvents.FIRE_AMBIENT, 1.0F + this.level.random.nextFloat(), this.level.random.nextFloat() * 0.7F + 0.3F));
                                    ModNetwork.sendToALL(serverWorld.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(this.worldPosition, SoundEvents.SCULK_CATALYST_BLOOM, 1.0F, this.level.random.nextFloat() * 0.1F + 0.9F));
                ...
                if (recipe != null) {
                    ...
                    if (finished) {
                        ItemStack itemstack1 = this.level.getRecipeManager()
                                .getRecipeFor(ModRecipeSerializer.BRAZIER_TYPE, this.getContainer(), this.level)
                        ...
                        this.level.playSound(null, this.getBlockPos(), ModSounds.CAST_SPELL, SoundSource.BLOCKS, 2.0F, 0.5F);
                    } else {
                        if (!this.isEmpty()) {
                            Containers.dropContents(this.level, this.getBlockPos(), this.getItems());
                            this.level.playSound(null, this.getBlockPos(), ModSounds.SPELL_FAIL, SoundSource.BLOCKS, 2.0F, 0.5F);
                        }
                    }
                } else {
                    if (!this.isEmpty()) {
            ...
            if (t % 20L == 0L) {
                if (this.getBlockState().getValue(NecroBrazierBlock.WATERLOGGED)) {
                    if (serverLevel.getFluidState(blockpos.above()).isEmpty() && serverLevel.getBlockState(blockpos.above()).isAir()) {
                        serverLevel.sendParticles(ModParticleTypes.NECRO_FIRE, blockpos.getX() + 0.5F, blockpos.getY() + 1.0F, blockpos.getZ() + 0.5F, 1, 0, 0, 0, 0);
                        serverLevel.sendParticles(ModParticleTypes.NECRO_FIRE_DROP, blockpos.getX() + 0.5F, blockpos.getY() + 1.0F, blockpos.getZ() + 0.5F, 1, 0, 0, 0, 0);
                    } else {
                        for (int p = 0; p < 4; ++p) {
