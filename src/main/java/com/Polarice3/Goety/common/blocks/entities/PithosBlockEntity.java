...
public class PithosBlockEntity extends RandomizableContainerBlockEntity implements IChunkLoader {
    ...
    public PithosBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(ModBlockEntities.PITHOS, blockPos, blockState);
    }
            ...
            for (int i = -range; i < range; ++i) {
                for (int j = -range; j < range; ++j) {
                    for (int k = -range; k < range; ++k) {
                        ...
                        if (blockState.is(ModBlocks.CRYPT_CHEST)) {
                            if (blockState.hasProperty(CryptChestBlock.LOCKED) && blockState.getValue(CryptChestBlock.LOCKED)) {
                                ...
                                if (this.level instanceof ServerLevel serverLevel) {
                                    for (int i1 = 0; i1 < serverLevel.random.nextInt(10) + 10; ++i1) {
                                        serverLevel.sendParticles(ModParticleTypes.SUMMON, this.getRandomX(1.5D, serverLevel.random), this.getRandomY(serverLevel.random), this.getRandomZ(1.5D, serverLevel.random), 0, 0.0F, 0.0F, 0.0F, 1.0F);
                                    }
                                }
                            }
                        }
