...
public class SoulAbsorberBlockEntity extends ModBlockEntity implements Clearable, WorldlyContainer {
    ...
    public SoulAbsorberBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(ModBlockEntities.SOUL_ABSORBER, blockPos, blockState);
    }
        ...
        if (!this.itemStack.isEmpty()) {
            if (this.itemStack.getItem() instanceof ITotem) {
                ...
                int soulIncrease = this.level.getRecipeManager()
                        .getRecipeFor(ModRecipeSerializer.SOUL_ABSORBER, iinventory, this.level)
    ...
    private boolean checkArca() {
        ...
        return this.level.getBlockState(new BlockPos(this.getBlockPos().getX(), this.getBlockPos().getY() - 1, this.getBlockPos().getZ())).is(ModBlocks.ARCA_BLOCK);
    }
    ...
    public Optional<SoulAbsorberRecipes> getRecipes(ItemStack pStack) {
        return this.level.getRecipeManager().getRecipeFor(ModRecipeSerializer.SOUL_ABSORBER, new SimpleContainer(pStack), this.level);
    }
