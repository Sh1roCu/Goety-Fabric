...
public class SoulCandlestickBlockEntity extends BlockEntity implements ISoulCandle {
    ...
    public SoulCandlestickBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.SOUL_CANDLESTICK, p_155229_, p_155230_);
    }
            ...
            if (this.checkCage()) {
                if (this.cursedCageTile.getSouls() > amount) {
                    ...
                    if (this.level instanceof ServerLevel serverLevel) {
                        serverLevel.sendParticles(ModParticleTypes.SOUL_EXPLODE_BITS, this.getBlockPos().getX() + 0.5D, this.getBlockPos().getY() + 0.75D, this.getBlockPos().getZ() + 0.5D, 0, d0, d1, d2, 0.5D);
                    }
                }
            }
        }
