...
public class SpiderMotherDenBlockEntity extends ModBlockEntity {
    ...
    public SpiderMotherDenBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.SPIDER_MOTHER_DEN, p_155229_, p_155230_);
    }
                        ...
                        for (int i = 0; i < serverLevel.getRandom().nextInt(4) + 1; ++i) {
                            ...
                            if (serverLevel.noCollision(EntityType.SPIDER.getAABB(d0, d1, d2))) {
                                ...
                                if (SpawnPlacements.checkSpawnRules(EntityType.SPIDER, serverLevel, MobSpawnType.SPAWNER, blockpos, serverLevel.getRandom())) {
                                    ...
                                    int k = this.level.getEntitiesOfClass(entity.getClass(), (new AABB(this.worldPosition.getX(), this.worldPosition.getY(), this.worldPosition.getZ(), this.worldPosition.getX() + 1, this.worldPosition.getY() + 1, this.worldPosition.getZ() + 1)).inflate(4.0D)).size();
                                    if (k >= 6) {
                                        this.delay(serverLevel);
                                        return;
                                    }
                                    ...
                                    if (entity instanceof Mob mob) {
                                        mob.finalizeSpawn(serverLevel, serverLevel.getCurrentDifficultyAt(this.worldPosition), MobSpawnType.SPAWNER, null, null);
                                        mob.spawnAnim();
                                    }
