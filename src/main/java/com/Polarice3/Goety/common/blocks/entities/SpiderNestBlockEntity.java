...
public class SpiderNestBlockEntity extends TrainingBlockEntity {
    ...
    public SpiderNestBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(ModBlockEntities.SPIDER_NEST, p_155229_, p_155230_);
    }
        ...
        if (this.isTraining()) {
            if (this.trainTime != this.getMaxTrainTime()) {
                ...
                if (this.trainTime == 20) {
                    level.playSound(null, this.getBlockPos(), ModSounds.SPIDER_NEST_TRAIN, SoundSource.BLOCKS, 1.0F, 1.0F);
                }
            } else {
                if (level instanceof ServerLevel serverLevel) {
                    if (level.random.nextInt(10) == 0) {
    ...
    public void setVariant(ItemStack itemStack, Level level, BlockPos blockPos) {
        if (level instanceof ServerLevel serverLevel) {
            if (BlockFinder.findStructure(serverLevel, blockPos, ModTags.Structures.CRYPT)
                    ...
                    && getBlocks(blockState -> blockState.is(ModBlocks.SKULL_PILE), 4))) {
                if (this.getTrainMob() != ModEntityType.BONE_SPIDER_SERVANT) {
                    this.setEntityType(ModEntityType.BONE_SPIDER_SERVANT);
                    this.markUpdated();
                }
            } else if (RitualRequirements.frostRitual(null, blockPos, level)) {
                if (this.getTrainMob() != ModEntityType.ICY_SPIDER_SERVANT) {
                    this.setEntityType(ModEntityType.ICY_SPIDER_SERVANT);
                    this.markUpdated();
                }
                    ...
                    // TODO: ConventionalBlockTags.STONE
                    && getBlocks(blockState -> blockState.is(/*ConventionalBlockTags.STONE*/Blocks.STONE), 8))) {
                if (this.getTrainMob() != ModEntityType.CAVE_SPIDER_SERVANT) {
                    this.setEntityType(ModEntityType.CAVE_SPIDER_SERVANT);
                    this.markUpdated();
                }
                ...
                if (this.getTrainMob() != ModEntityType.WEB_SPIDER_SERVANT) {
                    this.setEntityType(ModEntityType.WEB_SPIDER_SERVANT);
                    this.markUpdated();
                }
            } else {
                if (this.getTrainMob() != ModEntityType.SPIDER_SERVANT) {
                    this.setEntityType(ModEntityType.SPIDER_SERVANT);
                    this.markUpdated();
                }
            }
        }
    ...
    public void startTraining(int amount, ItemStack itemStack) {
        ...
        if (this.level != null) {
            this.level.playSound(null, this.getBlockPos(), ModSounds.SPIDER_NEST_START, SoundSource.BLOCKS, 1.0F, 1.0F);
        }
    }
    ...
    public void playSpawnSound() {
        if (this.level != null) {
            this.level.playSound(null, this.getBlockPos(), ModSounds.SPIDER_NEST_SPAWN, SoundSource.BLOCKS, 1.0F, 1.0F);
        }
    }
    ...
    public boolean isFuel(ItemStack itemStack) {
        var foodData = itemStack.getItem().getFoodProperties();
        return itemStack.getItem().isEdible() && this.getPlayer() != null && foodData != null && foodData.isMeat();
    }
}
