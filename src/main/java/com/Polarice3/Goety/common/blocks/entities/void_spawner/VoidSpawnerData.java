...
public class VoidSpawnerData {
    ...
    public static MapCodec<VoidSpawnerData> MAP_CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                            ModUUIDUtil.CODEC_SET.optionalFieldOf("registered_players", Sets.newHashSet()).forGetter(trialSpawnerData -> trialSpawnerData.detectedPlayers),
                            ModUUIDUtil.CODEC_SET.optionalFieldOf("current_mobs", Sets.newHashSet()).forGetter(trialSpawnerData -> trialSpawnerData.currentMobs),
    ...
    public void tryDetectPlayers(ServerLevel serverLevel, BlockPos blockPos, VoidSpawner voidSpawner) {
        ...
        if (bl) {
            ...
            serverLevel.playSound(null, blockPos, ModSounds.VOID_SPAWNER_DETECT_PLAYER, SoundSource.BLOCKS, 1.0F, (serverLevel.getRandom().nextFloat() - serverLevel.getRandom().nextFloat()) * 0.2F + 1.0F);
