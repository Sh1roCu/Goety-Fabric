...
public enum VoidSpawnerState implements StringRepresentable {
    ...
    VoidSpawnerState(String string2, int j, ParticleEmission particleEmission, double d, boolean bl) {
        this.name = string2;
        this.lightLevel = j;
        ...
        switch (this) {
            ...
            case WAITING_FOR_REWARD_EJECTION:
                if (data.isReadyToOpenShutter(serverLevel, config, 40.0F)) {
                    serverLevel.playSound(null, blockPos, ModSounds.VOID_SPAWNER_OPEN_SHUTTER, SoundSource.BLOCKS);
            ...
            case EJECTING_REWARD:
                if (!data.isReadyToEjectItems(serverLevel, config, (float) TIME_BETWEEN_EACH_EJECTION)) {
                    ...
                    serverLevel.playSound(null, blockPos, ModSounds.VOID_SPAWNER_CLOSE_SHUTTER, SoundSource.BLOCKS);
    ...
    interface ParticleEmission {
        ...
        ParticleEmission SMALL_FLAMES = (level, randomSource, blockPos) -> {
            if (randomSource.nextInt(2) == 0) {
                ...
                addParticle(ModParticleTypes.SMALL_END_FIRE, vec3, level);
            }
        };
        ParticleEmission FLAMES_AND_SMOKE = (level, randomSource, blockPos) -> {
            ...
            addParticle(ModParticleTypes.END_FIRE, vec3, level);
            addParticle(ModParticleTypes.END_FIRE_DROP, vec3, level);
        };
