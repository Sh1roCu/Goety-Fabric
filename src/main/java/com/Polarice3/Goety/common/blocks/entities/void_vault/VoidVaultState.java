...
public enum VoidVaultState implements StringRepresentable {
    INACTIVE("inactive", Light.HALF_LIT) {
        ...
        protected void onChangedTo(ServerLevel world, BlockPos pos, VoidVaultConfig config, VoidVaultSharedData sharedData) {
            ...
            world.playSound(null, pos, ModSounds.VOID_VAULT_DEACTIVATE, SoundSource.BLOCKS, 1.0F, (world.getRandom().nextFloat() - world.getRandom().nextFloat()) * 0.2F + 1.0F);
        }
    },
    ACTIVE("active", Light.LIT) {
        ...
        protected void onChangedTo(ServerLevel world, BlockPos pos, VoidVaultConfig config, VoidVaultSharedData sharedData) {
            ...
            if (world.getBlockEntity(pos) instanceof VoidVaultBlockEntity vaultBlockEntity) {
                ...
                world.playSound(null, pos, ModSounds.VOID_VAULT_ACTIVATE, SoundSource.BLOCKS, 1.0F, (world.getRandom().nextFloat() - world.getRandom().nextFloat()) * 0.2F + 1.0F);
            }
    ...
    UNLOCKING("unlocking", Light.LIT) {
        @Override
        protected void onChangedTo(ServerLevel world, BlockPos pos, VoidVaultConfig config, VoidVaultSharedData sharedData) {
            world.playSound(null, pos, ModSounds.VOID_VAULT_INSERT, SoundSource.BLOCKS);
        }
    },
    EJECTING("ejecting", Light.LIT) {
        @Override
        protected void onChangedTo(ServerLevel world, BlockPos pos, VoidVaultConfig config, VoidVaultSharedData sharedData) {
            world.playSound(null, pos, ModSounds.VOID_VAULT_OPEN_SHUTTER, SoundSource.BLOCKS);
        }

        @Override
        protected void onChangedFrom(ServerLevel world, BlockPos pos, VoidVaultConfig config, VoidVaultSharedData sharedData) {
            world.playSound(null, pos, ModSounds.VOID_VAULT_CLOSE_SHUTTER, SoundSource.BLOCKS);
        }
    };
    private final String id;
    private final Light light;

    VoidVaultState(String id, Light light) {
        this.id = id;
        this.light = light;
    }
    ...
    private void ejectItem(ServerLevel world, BlockPos pos, ItemStack stack, float pitchModifier) {
        ...
        world.playSound(null, pos, ModSounds.VOID_VAULT_EJECT_ITEM, SoundSource.BLOCKS, 1.0F, 0.8F + 0.4F * pitchModifier);
    }
