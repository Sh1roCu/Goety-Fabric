    ...
    public void applyEffectTick(LivingEntity living, int amplify) {
        if (living.isAlive()) {
            if (living.level instanceof ServerLevel worldIn) {
                ...
                if (flag) {
                    worldIn.sendParticles(ModParticleTypes.SPELL_ELECTRIC, living.getRandomX(0.5D), living.getRandomY(), living.getRandomZ(0.5D), 1, 0.0D, 0.5D, 0.0D, 0);
                }
                if (living.tickCount % 80 == 0) {
                    ...
                    while (iterator.hasNext() && i < 2) {
                        ...
                        if (entity instanceof LivingEntity target) {
                            ...
                            ModNetwork.sendToALL(worldIn.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, vec31, 10));
                            ...
                            worldIn.playSound(null, living.getX(), living.getY(), living.getZ(), ModSounds.ZAP, living.getSoundSource(), 1.0F, 1.0F);
                            ++i;
                        }
                    }
                }
