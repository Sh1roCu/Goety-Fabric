...
public class GoetyBaseEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity livingEntity, int amplify) {
        ...
        if (this == GoetyEffects.STORMS_WRATH) {
            float chance = 0.1F;
        ...
        if (this == GoetyEffects.PHOTOSYNTHESIS) {
        ...
        if (this == GoetyEffects.ARROWMANTIC) {
        ...
        if (this == GoetyEffects.FIERY_AURA) {
                ...
                for (LivingEntity living : world.getEntitiesOfClass(LivingEntity.class, livingEntity.getBoundingBox().inflate(f))) {
                    if (!living.isOnFire() && !living.hasEffect(MobEffects.FIRE_RESISTANCE) && !living.fireImmune() && !MobUtil.areAllies(livingEntity, living) && MobUtil.validEntity(living) && living != livingEntity) {
                        ...
                        if (living.getRemainingFireTicks() > 0) {
                            ...
                            ModNetwork.sendToALL(serverLevel.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(livingEntity.blockPosition(), SoundEvents.FIRECHARGE_USE, 1.0F, 0.75F));
                        }
                    }
                }
            }
        }
        if (this == GoetyEffects.FROSTY_AURA) {
            if (world instanceof ServerLevel serverLevel) {
                float f = 2.0F + amplify;
                ...
                for (LivingEntity living : world.getEntitiesOfClass(LivingEntity.class, livingEntity.getBoundingBox().inflate(f))) {
                    if (!living.isFreezing() && living.canFreeze() && !MobUtil.areAllies(livingEntity, living) && MobUtil.validEntity(living) && living != livingEntity) {
                        if (living.addEffect(new MobEffectInstance(GoetyEffects.FREEZING, 100, amplify))) {
                            ServerParticleUtil.addParticlesAroundSelf(serverLevel, ParticleTypes.SNOWFLAKE, living);
                            ModNetwork.sendToALL(serverLevel.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(livingEntity.blockPosition(), SoundEvents.PLAYER_HURT_FREEZE, 1.0F, 0.75F));
                        }
                    }
                }
            }
        }
        if (this == GoetyEffects.CHILL_HIDE) {
            if (world instanceof ServerLevel serverLevel) {
        ...
        if (this == GoetyEffects.FIRE_TRAIL) {
        ...
        if (this == GoetyEffects.GRAVITY_PULSE) {
                ...
                if (world instanceof ServerLevel serverLevel) {
                    ...
                    for (LivingEntity target : livingEntity.level.getEntitiesOfClass(LivingEntity.class, livingEntity.getBoundingBox().inflate(area))) {
                        if (target != livingEntity && !MobUtil.areAllies(livingEntity, target) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(target) && target != livingEntity.getVehicle()) {
                            ...
                            target.level.playSound(null, target.getX(), target.getY(), target.getZ(), ModSounds.GRAVITY, livingEntity.getSoundSource(), 0.65F, 0.9F + (livingEntity.getRandom().nextFloat() * 0.4F));
        ...
        /*if (this == GoetyEffects.SOUL_BURN)){
        ...
        if (this == GoetyEffects.CRIPPLED) {
            if (livingEntity.getHealth() >= livingEntity.getMaxHealth() * 0.75F) {
                livingEntity.removeEffect(GoetyEffects.CRIPPLED);
            }
        }
        if (this == GoetyEffects.STUNNED) {
            if (livingEntity.level instanceof ServerLevel serverLevel) {
                if (livingEntity.tickCount % 15 == 0) {
                    serverLevel.sendParticles(ModParticleTypes.STUN, livingEntity.getX(), livingEntity.getY() + livingEntity.getBbHeight() + 0.5F, livingEntity.getZ(), 1, 0, 0, 0, 0.5F);
                }
            }
        }
        if (this == GoetyEffects.WILD_RAGE) {
    ...
    public boolean isDurationEffectTick(int tick, int amplify) {
        ...
        if (this == GoetyEffects.TRIPPING
                || this == GoetyEffects.STORMS_WRATH) {
            int j = 20 >> amplify;
            if (j > 0) {
                return tick % j == 0;
            }
