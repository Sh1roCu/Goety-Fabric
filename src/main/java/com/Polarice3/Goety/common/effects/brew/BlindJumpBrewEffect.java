    ...
    public void applyEntityEffect(LivingEntity pTarget, @Nullable Entity pSource, @Nullable Entity pIndirectSource, int pAmplifier) {
        if (!pTarget.level.isClientSide) {
            for (int i = 0; i < 128; ++i) {
                ...
                double d1 = Mth.clamp(pTarget.getY() + (pTarget.getRandom().nextInt((16 * d)) - (8.0D * d)), pTarget.level.getMinBuildHeight(), pTarget.level.getMinBuildHeight() + ((ServerLevel) pTarget.level).getLogicalHeight() - 1);
                double d2 = pTarget.getZ() + (pTarget.getRandom().nextDouble() - 0.5D) * (16.0D * d);
                var event = EventHooks.onEnderTeleport(pTarget, d0, d1, d2);
                if (event.isCanceled()) {
                    break;
                }
                if (pTarget.randomTeleport(event.getTargetX(), event.getTargetY(), event.getTargetZ(), true)) {
                    ...
                    if (!pTarget.isSilent()) {
                        pTarget.level.playSound(null, pTarget.xo, pTarget.yo, pTarget.zo, SoundEvents.ENDERMAN_TELEPORT, pTarget.getSoundSource(), 1.0F, 1.0F);
