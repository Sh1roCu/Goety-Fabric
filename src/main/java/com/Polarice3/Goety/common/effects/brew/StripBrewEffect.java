            ...
            if (!armor.isEmpty()) {
                ...
                if (pTarget.level.addFreshEntity(itemEntity)/* || item.hasCustomEntity(itemStack)*/) {
                    ...
                    if (pTarget.level instanceof ServerLevel serverLevel) {
                        ModNetwork.sendToALL(serverLevel.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(pTarget.blockPosition(), SoundEvents.ARMOR_EQUIP_GENERIC, 10.0F, 1.5F));
                    }
