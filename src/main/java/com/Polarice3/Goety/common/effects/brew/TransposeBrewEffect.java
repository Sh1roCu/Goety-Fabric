            ...
            for (Entity entity : pLevel.getEntitiesOfClass(Entity.class, new AABB(blockPos))) {
                if (entity instanceof ItemEntity || entity instanceof LivingEntity) {
                    if (entity instanceof LivingEntity livingEntity) {
                        var event = EventHooks.onEnderTeleport(livingEntity, pPos.getX(), pPos.getY() + 1, pPos.getZ());
                        if (event.isCanceled()) {
                            break;
                        }
                        ...
                        if (!entity.isSilent()) {
                            pLevel.playSound(null, entity.xo, entity.yo, entity.zo, SoundEvents.ENDERMAN_TELEPORT, entity.getSoundSource(), 1.0F, 1.0F);
