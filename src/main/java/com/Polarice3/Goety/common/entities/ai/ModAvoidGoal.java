...
public class ModAvoidGoal<T extends LivingEntity> extends Goal {
    ...
    public boolean canUse() {
        this.toAvoid = this.mob.level.getNearestEntity(this.mob.level.getEntitiesOfClass(this.avoidClass, this.mob.getBoundingBox().inflate(this.avoidStart, 3.0D, this.avoidStart), (p_148078_) -> {
            return true;
        }), this.avoidEntityTargeting, this.mob, this.mob.getX(), this.mob.getY(), this.mob.getZ());
        if (this.toAvoid == null) {
            return false;
        } else {
            Vec3 vec3 = DefaultRandomPos.getPosAway(this.mob, this.maxDist, this.minDist, this.toAvoid.position());
            if (vec3 == null) {
                return false;
