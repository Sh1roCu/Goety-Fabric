...
public class SlimeServant extends Summoned {
    ...
    public EntityType<?> getVariant(Level level, BlockPos blockPos) {
        EntityType<?> entityType = ModEntityType.SLIME_SERVANT;
        if (level instanceof ServerLevel serverLevel) {
            if (level.isWaterAt(blockPos)) {
                entityType = ModEntityType.TROPICAL_SLIME_SERVANT;
            } else if (level.dimension() == Level.NETHER) {
                entityType = ModEntityType.MAGMA_CUBE_SERVANT;
            } else if (BlockFinder.findStructure(serverLevel, blockPos, ModTags.Structures.CRYPT)) {
                entityType = ModEntityType.CRYPT_SLIME_SERVANT;
            }
        }
        return entityType;
    }
