    ...
    public void tick() {
        ...
        if (this.level instanceof ServerLevel serverLevel) {
            if (this.tickCount % 5 == 0 || this.attackTime > 0) {
                serverLevel.sendParticles(ModParticleTypes.SPELL_ELECTRIC, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 1, 0.0D, 0.0D, 0.0D, 0.0D);
            }
            if (this.isStaying()) {
    ...
    public void die(DamageSource pCause) {
        ...
        if (this.level instanceof ServerLevel serverLevel) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                ModNetwork.sendToALL(serverLevel.getServer(), SLightningPacket.ID, SLightningPacket.encode(vec3, vec31, colorUtil, 8));
            }
        }
        this.discard();
    }
            ...
            if (livingentity != null && livingentity.distanceTo(this.spriteMob) < attackRange && this.spriteMob.hasLineOfSight(livingentity)) {
                ...
                if (this.spriteMob.attackTime == 20) {
                    ...
                    if (flag) {
                        ...
                        spell.mobSpellResult(this.spriteMob, this.spriteMob.isUpgraded() ? ModItems.STORM_STAFF.getDefaultInstance() : ItemStack.EMPTY, WandUtil.getStats(this.spriteMob, spell).setRange((int) attackRange).increasePotency(potency));
                    }
