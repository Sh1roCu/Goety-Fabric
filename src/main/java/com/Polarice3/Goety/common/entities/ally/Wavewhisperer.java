...
//    }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.WAVEWHISPERER_AMBIENT;
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource p_21239_) {
        return ModSounds.WAVEWHISPERER_HURT;
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.WAVEWHISPERER_DEATH;
    }

    @Override
    protected void playStepSound(BlockPos p_20135_, BlockState p_20136_) {
        this.playSound(ModSounds.WAVEWHISPERER_STEP, 0.15F, 1.0F);
    }

    @Override
    protected SoundEvent getAttackSound() {
        return ModSounds.WAVEWHISPERER_ATTACK;
    }

    @Override
    protected SoundEvent getSummonSound() {
        return ModSounds.WAVEWHISPERER_SUMMON;
    }

    @Override
    protected SoundEvent getSummonPoisonSound() {
        return ModSounds.WAVEWHISPERER_SUMMON_POISON;
    }

    @Override
    protected SoundEvent getSummonThornsSound() {
        return ModSounds.WAVEWHISPERER_SUMMON_THORNS;
    }

    @Override
    protected EntityType<? extends AbstractMonolith> getVines() {
        return ModEntityType.QUICK_GROWING_KELP;
    }

    @Override
    protected EntityType<? extends AbstractMonolith> getPoison() {
        return ModEntityType.POISON_ANEMONE;
    }
    ...
    static class SwimUpGoal extends Goal {
        ...
        public void tick() {
            if (this.whisperer.getY() < (double) (this.seaLevel - 1) && (this.whisperer.getNavigation().isDone() || this.whisperer.closeToNextPos())) {
                Vec3 vec3 = DefaultRandomPos.getPosTowards(this.whisperer, 4, 8, new Vec3(this.whisperer.getX(), this.seaLevel - 1, this.whisperer.getZ()), (float) Math.PI / 2F);
                if (vec3 == null) {
                    this.stuck = true;
                    return;
                }
