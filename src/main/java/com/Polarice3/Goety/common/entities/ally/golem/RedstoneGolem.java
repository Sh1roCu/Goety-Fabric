...
public class RedstoneGolem extends RaiderGolemServant {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                ...
                .add(StepHeightEntityAttributeMain.STEP_HEIGHT, 1.0D)
    ...
    public SoundEvent getCelebrateSound() {
        return ModSounds.REDSTONE_GOLEM_AMBIENT;
    }
    ...
    protected SoundEvent getHurtSound(DamageSource p_21239_) {
        return ModSounds.REDSTONE_GOLEM_HURT;
    }

    @Override
    protected void playStepSound(BlockPos p_20135_, BlockState p_20136_) {
        this.playSound(ModSounds.REDSTONE_GOLEM_STEP);
    ...
    protected SoundEvent getDeathSound() {
        return ModSounds.REDSTONE_GOLEM_DEATH;
    }
    ...
    protected void tickDeath() {
        ...
        if (this.deathTime >= 30) {
            this.spawnAnim();
            ItemStack itemStack = new ItemStack(ModBlocks.REDSTONE_GOLEM_SKULL_ITEM);
        ...
        if (this.isHostile()) {
            if (MobsConfig.RedstoneGolemHostileConvert.get()) {
                if (this.tickCount % 10 == 0) {
                    Mob mob = this.convertTo(ModEntityType.HOSTILE_REDSTONE_GOLEM, false);
                    if (mob != null) {
                ...
                if (this.isAlive() && !this.isActivating()) {
                    if (this.isNovelty) {
                        ...
                        if (this.noveltyTick == 8 || this.noveltyTick == 13 || this.noveltyTick == 18 || this.noveltyTick == 23 || this.noveltyTick == 28 || this.noveltyTick == 33) {
                            this.playSound(ModSounds.REDSTONE_GOLEM_CHEST);
                        }
                        if (this.noveltyTick == 42) {
                            this.playSound(ModSounds.REDSTONE_GOLEM_GROWL);
                    ...
                    if (this.isSummoning()) {
                        if (this.level instanceof ServerLevel serverLevel) {
                            for (int i = 0; i < 5; ++i) {
                                ...
                                serverLevel.sendParticles(ModParticleTypes.BIG_ELECTRIC, this.getRandomX(0.5D), this.getEyeY() - serverLevel.random.nextInt(5), this.getRandomZ(0.5D), 0, d0, d1, d2, 0.5F);
                            }
                        }
                        ...
                        if (this.summonTick <= (MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 1)) && this.mineCount > 0) {
                            int time = MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 1) / 14;
    ...
    public void handleEntityEvent(byte pId) {
        if (pId == 4) {
            this.playSound(ModSounds.REDSTONE_GOLEM_SUMMON);
            this.summonTick = MathHelper.secondsToTicks(SUMMON_SECONDS_TIME) + 5;
            this.getGlow = 1.0F;
        } else if (pId == 5) {
            this.attackTick = 0;
        } else if (pId == 6) {
            this.playSound(ModSounds.REDSTONE_GOLEM_ATTACK);
        } else if (pId == 7) {
            this.deathRotation = this.getYRot();
            this.playSound(ModSounds.REDSTONE_GOLEM_DEATH, 1.0F, 1.0F);
        } else if (pId == 12) {
            this.isSittingDown = MathHelper.secondsToTicks(1);
        } else if (pId == 13) {
            this.isStandingUp = MathHelper.secondsToTicks(1);
    ...
    public boolean doHurtTarget(Entity entityIn) {
        if (!this.level.isClientSide && !this.isMeleeAttacking()) {
            this.playSound(ModSounds.REDSTONE_GOLEM_PRE_ATTACK, 1.5F, 1.0F);
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.getTrueOwner() != null && pPlayer == this.getTrueOwner()) {
                if ((itemstack.is(ModBlocks.REINFORCED_REDSTONE_BLOCK.asItem())
                        || itemstack.is(Items.REDSTONE_BLOCK/*Tags.Items.STORAGE_BLOCKS_REDSTONE*/)
                        || itemstack.is(ConventionalItemTags.REDSTONE_DUSTS))
                    ...
                    } else if (itemstack.is(Items.REDSTONE_BLOCK/*Tags.Items.STORAGE_BLOCKS_REDSTONE*/)) {
            ...
            if (RedstoneGolem.this.getTarget() != null && RedstoneGolem.this.getTarget().isAlive()) {
                ...
                if (RedstoneGolem.this.targetClose(livingentity, d0)) {
                    ...
                    if (RedstoneGolem.this.attackTick == 1) {
                        RedstoneGolem.this.playSound(ModSounds.REDSTONE_GOLEM_ATTACK);
            ...
            if (flag) {
                if (f1 > 0.0F && target instanceof LivingEntity livingEntity) {
                    if (livingEntity.getBoundingBox().getSize() > RedstoneGolem.this.getBoundingBox().getSize()) {
                        livingEntity.knockback(f1 * 0.5F, Mth.sin(RedstoneGolem.this.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(RedstoneGolem.this.getYRot() * ((float) Math.PI / 180F)));
                    } else {
                        MobUtil.forcefulKnockBack(livingEntity, f1 * 0.5F, Mth.sin(RedstoneGolem.this.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(RedstoneGolem.this.getYRot() * ((float) Math.PI / 180F)), 0.5D);
                    }
    ...
    public class SummonMinesGoal extends Goal {
        ...
        public void start() {
            super.start();
            RedstoneGolem.this.playSound(ModSounds.REDSTONE_GOLEM_SUMMON, RedstoneGolem.this.getSoundVolume(), RedstoneGolem.this.getVoicePitch());
            RedstoneGolem.this.setAnimationState(SUMMON);
            RedstoneGolem.this.summonTick = MathHelper.secondsToTicks(SUMMON_SECONDS_TIME);
            RedstoneGolem.this.summonCool = MathHelper.secondsToTicks(10 + SUMMON_SECONDS_TIME);
