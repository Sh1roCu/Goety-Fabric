...
public class RedstoneMonstrosity extends RaiderGolemServant implements PlayerRideable, IAutoRideable, IRM {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.REDSTONE_MONSTROSITY_AMBIENT;
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource p_21239_) {
        return ModSounds.REDSTONE_MONSTROSITY_HURT;
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.REDSTONE_MONSTROSITY_DEATH;
    }

    @Override
    protected void playStepSound(BlockPos p_20135_, BlockState p_20136_) {
        if (this.isAggressive()) {
            this.playSound(ModSounds.REDSTONE_MONSTROSITY_CHASE, 2.0F, 1.0F);
        } else {
            this.playSound(ModSounds.REDSTONE_MONSTROSITY_STEP, 2.0F, 1.0F);
        }
    ...
    protected void tickDeath() {
        ...
        if (this.deathTime >= MathHelper.secondsToTicks(5)) {
            ...
            if (this.getTrueOwner() != null) {
                ItemStack itemStack = new ItemStack(ModBlocks.REDSTONE_MONSTROSITY_HEAD_BLOCK);
    ...
    public void tick() {
        ...
        if (this.isHostile()) {
            if (MobsConfig.RedstoneMonstrosityHostileConvert.get()) {
                if (this.tickCount % 10 == 0) {
                    Mob mob = this.convertTo(ModEntityType.HOSTILE_REDSTONE_MONSTROSITY, false);
                    if (mob != null) {
        ...
        if (this.hasPose(Pose.EMERGING)) {
            ...
            if (this.activateTick == 1) {
                this.playSound(ModSounds.REDSTONE_MONSTROSITY_AWAKEN, 10.0F, 1.0F);
            }
                ...
                if (this.isSummoning()) {
                    ...
                    if (this.level instanceof ServerLevel serverLevel) {
                        for (int i = 0; i < 5; ++i) {
                            ...
                            serverLevel.sendParticles(ModParticleTypes.ELECTRIC, this.getRandomX(0.5D), this.getEyeY() - serverLevel.random.nextInt(2), this.getRandomZ(0.5D), 0, d0, d1, d2, 0.5F);
                        }
                        if (this.summonTick < MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 1)
                            ...
                            if (!this.level.getBlockState(this.blockPosition().below()).isAir()) {
                                for (int j = 0; j < 4; ++j) {
                                    ...
                                    serverLevel.sendParticles(ModParticleTypes.BIG_FIRE_GROUND, d1, d2, d3, 0, 0.0D, 0.0D, 0.0D, 0.5F);
                                }
                            }
                        }
                        if (this.summonTick == MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 1)) {
                                ...
                                if (target != this && !MobUtil.areAllies(this, target)) {
                                    if (target.hurt(this.getServantAttack(), 2.0F)) {
                                        ...
                                        if (target.getBoundingBox().getSize() > this.getBoundingBox().getSize()) {
                                            target.knockback(f1 * 0.5F, Mth.sin(this.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(this.getYRot() * ((float) Math.PI / 180F)));
                                        } else {
                                            MobUtil.forcefulKnockBack(target, f1 * 0.5F, Mth.sin(this.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(this.getYRot() * ((float) Math.PI / 180F)), 0.5D);
                                        }
                                    }
                                }
                            }
                        ...
                        if (this.summonTick <= (MathHelper.secondsToTicks(SUMMON_SECONDS_TIME - 2)) && this.summonCount != 0) {
                            for (int i = 0; i < 10; ++i) {
                                ...
                                Summoned summoned = new RedstoneCube(ModEntityType.REDSTONE_CUBE, this.level);
                                if (this.level.noCollision(summoned, summoned.getBoundingBox().move(vec3))) {
                                    ...
                                    summonCircle.playSound(ModSounds.DIRT_DEBRIS, 3.0F, 0.4F);
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        if (!this.level.isClientSide) {
            ...
            if (this.getTrueOwner() != null && pPlayer == this.getTrueOwner()) {
                if ((itemstack.is(Items.REDSTONE_BLOCK/*Tags.Items.STORAGE_BLOCKS_REDSTONE*/)
                        || itemstack.is(ModBlocks.REINFORCED_REDSTONE_BLOCK.asItem()))
                        && this.getHealth() < this.getMaxHealth()) {
                    if (!pPlayer.getAbilities().instabuild) {
                        itemstack.shrink(1);
                    }
                    if (itemstack.is(ModBlocks.REINFORCED_REDSTONE_BLOCK.asItem())) {
                    ...
                    if (this.level instanceof ServerLevel serverLevel) {
                        for (int i = 0; i < 7; ++i) {
                            ...
                            double d1 = serverLevel.random.nextGaussian() * 0.02D;
                            double d2 = serverLevel.random.nextGaussian() * 0.02D;
                            serverLevel.sendParticles(ModParticleTypes.HEAL_EFFECT, this.getRandomX(1.0D), this.getRandomY(), this.getRandomZ(1.0D), 0, d0, d1, d2, 0.5F);
                        }
                    }
    ...
    static class MeleeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (this.mob.attackTick > 0 && this.mob.getCurrentAnimation() != this.mob.getAnimationState(ATTACK)) {
                this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_GROWL);
                this.mob.setAnimationState(ATTACK);
            }
            if (this.mob.attackTick == 34) {
                this.mob.makeBigGlow();
                this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_SMASH, this.mob.getSoundVolume() * 2.0F, 0.2F);
            ...
            if (flag) {
                if (f1 > 0.0F && target instanceof LivingEntity livingEntity) {
                    if (livingEntity.getBoundingBox().getSize() > this.mob.getBoundingBox().getSize()) {
                        livingEntity.knockback(f1 * 0.5F, Mth.sin(this.mob.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(this.mob.getYRot() * ((float) Math.PI / 180F)));
                    } else {
                        MobUtil.forcefulKnockBack(livingEntity, f1 * 0.5F, Mth.sin(this.mob.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(this.mob.getYRot() * ((float) Math.PI / 180F)), 0.5D);
                    }
        ...
        public void tick() {
            ...
            if (!this.mob.isMeleeAttacking() && !this.mob.isSummoning()) {
                ...
                if (this.attackTime == 1) {
                    this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_ARM, this.mob.getSoundVolume() * 0.7F, this.mob.getVoicePitch());
                    ...
                    this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_BELCH, this.mob.getSoundVolume(), 0.7F);
                    this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_GROWL, this.mob.getSoundVolume() - 1.5F, 0.9F);
    ...
    public static class SummonGoal extends Goal {
        ...
        public void start() {
            ...
            this.mob.playSound(ModSounds.REDSTONE_MONSTROSITY_CHARGE);
