...
public class SquallGolem extends AbstractGolemServant implements IWindPowered {
    ...
    protected SoundEvent getHurtSound(@NotNull DamageSource p_21239_) {
        return ModSounds.SQUALL_GOLEM_HURT;
    }

    @Override
    protected void playStepSound(@NotNull BlockPos p_20135_, @NotNull BlockState p_20136_) {
        this.playSound(ModSounds.SQUALL_GOLEM_STEP, 1.0F, 1.0F);
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SQUALL_GOLEM_DEATH;
    }
            ...
            if (this.isAlive()) {
                if (this.isStartingUp() || this.isActivated()) {
                    if (this.tickCount % 7 == 0) {
                        if (!this.isAggressive()) {
                            this.playSound(ModSounds.SQUALL_GOLEM_WIND_SLOW);
                        } else {
                            this.playSound(ModSounds.SQUALL_GOLEM_WIND_FAST);
                        }
                    }
                }
                if (this.isProximity()) {
                ...
                if (this.isStartingUp() && !this.isActivated()) {
                    ...
                    if (this.startingUpTick == 1) {
                        this.playSound(ModSounds.SQUALL_GOLEM_ACTIVATE, 2.0F, 1.0F);
                        this.playSound(ModSounds.SQUALL_GOLEM_WIND_START);
                    }
                    ...
                    if (this.shuttingDownTick == 1) {
                        this.playSound(ModSounds.SQUALL_GOLEM_DEACTIVATE, 2.0F, 1.0F);
                        this.playSound(ModSounds.SQUALL_GOLEM_WIND_START);
                    }
    ...
    public @NotNull InteractionResult mobInteract(@NotNull Player pPlayer, @NotNull InteractionHand pHand) {
        if (!this.level.isClientSide) {
            ...
            if (this.getTrueOwner() != null && pPlayer == this.getTrueOwner()) {
                if ((item == ModBlocks.JADE_BLOCK.asItem() || item == ModItems.JADE) && this.getHealth() < this.getMaxHealth()) {
                    if (!pPlayer.getAbilities().instabuild) {
                        itemstack.shrink(1);
                    }
                    if (item == ModBlocks.JADE_BLOCK.asItem()) {
    ...
    class MeleeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (SquallGolem.this.attackTick == 1) {
                SquallGolem.this.playSound(ModSounds.SQUALL_GOLEM_ATTACK, 5.0F, 1.0F);
