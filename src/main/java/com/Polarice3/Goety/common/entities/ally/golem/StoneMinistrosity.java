...
public class StoneMinistrosity extends RaiderGolemServant {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.MINISTROSITY_STONE_IDLE;
    }
    ...
    protected SoundEvent getDeathSound() {
        return ModSounds.MINISTROSITY_DEATH;
    }

    @Override
    protected void playStepSound(BlockPos p_20135_, BlockState p_20136_) {
        this.playSound(ModSounds.MINISTROSITY_STEP, 0.15F, 1.0F);
    }
    ...
    public void setUpgraded(boolean upgraded) {
        if (this.getType() == ModEntityType.STONE_MINISTROSITY) {
            if (upgraded) {
                Entity entity = MobUtil.convertTo(this, ModEntityType.REDSTONE_MINISTROSITY, false, this.getTrueOwner() instanceof Player player ? player : null);
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand p_230254_2_) {
        if (!this.level.isClientSide) {
            ...
            if (this.getTrueOwner() != null && pPlayer == this.getTrueOwner()) {
                if (itemstack.is(Items.STONE/*Tags.Items.STONE*/) && this.getHealth() < this.getMaxHealth()) {
