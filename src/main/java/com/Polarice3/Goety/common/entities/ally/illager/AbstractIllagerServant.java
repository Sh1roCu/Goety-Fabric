...
public abstract class AbstractIllagerServant extends RaiderServant implements ITrainable, ILooter {
    ...
    public boolean canBreed() {
        ...
        if (!MobsConfig.IllagerServantAllBreed.get()) {
            flag = this.getType() == ModEntityType.NEOLLAGER;
        }
        return flag
    ...
    public Map<Item, Integer> getFoodPoints() {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (foodProperties != null) {
                if (this.validFood(itemStack) && this.canEat(itemStack)) {
    ...
    public AbstractIllagerServant getBreedOffspring(ServerLevel serverLevel, AbstractIllagerServant illager) {
        Neollager neollager = new Neollager(ModEntityType.NEOLLAGER, serverLevel);
        neollager.copyTrueOwner(this);
        neollager.finalizeSpawn(serverLevel, serverLevel.getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.BREEDING, null, null);
