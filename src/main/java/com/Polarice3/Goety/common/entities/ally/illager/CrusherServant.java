...
public class CrusherServant extends AbstractIllagerServant implements IMobCrafter {
    ...
    protected ResourceLocation getDefaultLootTable() {
        if (this.isNatural()) {
            return ModEntityType.CRUSHER.getDefaultLootTable();
        } else {
    ...
    public double getAttackReachSqr(LivingEntity enemy) {
        return this.getBbWidth() * 4.0F * this.getBbWidth() * 4.0F + enemy.getBbWidth();
    }
    ...
    private static CraftingRecipe getOminousSaddleRecipe(ServerLevel level) {
        if (OMINOUS_SADDLE_RECIPE == null) {
            for (CraftingRecipe recipe : level.getRecipeManager().getAllRecipesFor(RecipeType.CRAFTING)) {
                if (recipe.getResultItem(level.registryAccess()).is(ModItems.OMINOUS_SADDLE)) {
    ...
    private static CraftingRecipe getPuttyRecipe(ServerLevel level) {
        if (PUTTY_RECIPE == null) {
            for (CraftingRecipe recipe : level.getRecipeManager().getAllRecipesFor(RecipeType.CRAFTING)) {
                if (recipe.getResultItem(level.registryAccess()).is(ModItems.REPAIR_PUTTY)) {
                    PUTTY_RECIPE = recipe;
                    break;
                }
            }
    ...
    public static boolean canBeSmelted(ItemStack stack, ServerLevel level) {
        ...
        return !canSmelt(stack, level).isEmpty() && (stack.is(ConventionalItemTags.RAW_ORES/*Tags.Items.RAW_MATERIALS*/) || stack.is(ConventionalItemTags.ORES/*Tags.Items.ORES*/) || stack.is(ModTags.Items.CRUSHER_CAN_SMELT));
    }
    ...
    public boolean validLootToStore(ItemStack itemStack) {
        ...
        if (itemStack.is(ModItems.OMINOUS_SADDLE)) {
            return false;
        }
        if (itemStack.is(ModItems.REPAIR_PUTTY)) {
            return false;
        }
        boolean flag = super.validLootToStore(itemStack);
        if (!itemStack.is(ConventionalItemTags.RAW_ORES/*Tags.Items.RAW_MATERIALS*/) && !itemStack.is(ConventionalItemTags.ORES/*Tags.Items.ORES*/)) {
    ...
    public static boolean validCraft(ItemStack itemStack) {
        ...
        return validWeapon(itemStack) || itemStack.getItem() instanceof ArmorItem /*|| itemStack.is(Tags.Items.ARMORS)*/ || itemStack.is(ModItems.OMINOUS_SADDLE) || itemStack.is(ModItems.REPAIR_PUTTY);
    }
    ...
    class MeleeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (CrusherServant.this.attackTick == 11) {
                CrusherServant.this.playSound(ModSounds.HAMMER_SWING);
            }
            if (CrusherServant.this.attackTick == 13) {
                ...
                CrusherServant.this.playSound(ModSounds.HAMMER_IMPACT);
                CrusherServant.this.playSound(ModSounds.DIRT_DEBRIS);
                if (CrusherServant.this.isStorm()) {
                    CrusherServant.this.playSound(ModSounds.THUNDER_STRIKE_FAST);
                }
            ...
            if (CrusherServant.this.level instanceof ServerLevel serverLevel) {
                if (CrusherServant.this.isStorm() && target instanceof LivingEntity livingEntity) {
                    ...
                    if (lightningRod.isPresent()) {
                        ...
                        ModNetwork.sendToALL(serverLevel.getServer(), SLightningBoltPacket.ID, SLightningBoltPacket.encode(new Vec3(blockPos1.getX(), blockPos1.getY() + 250, blockPos1.getZ()), new Vec3(blockPos1.getX(), blockPos1.getY(), blockPos1.getZ()), 10));
                        serverLevel.playSound(null, CrusherServant.this.getX(), CrusherServant.this.getY(), CrusherServant.this.getZ(), ModSounds.THUNDERBOLT, CrusherServant.this.getSoundSource(), 1.0F, 1.0F);
                    } else {
                        Vec3 vec31 = new Vec3(livingEntity.getX(), livingEntity.getY() + livingEntity.getBbHeight() / 2, livingEntity.getZ());
                        ModNetwork.sendToALL(serverLevel.getServer(), SLightningBoltPacket.ID, SLightningBoltPacket.encode(new Vec3(livingEntity.getX(), livingEntity.getY() + 250, livingEntity.getZ()), vec31, 10));
                ...
                if (!entities.isEmpty()) {
                    ...
                    if (target.hurt(ModDamageSource.directShock(pAttacker), damage)) {
                        if (prevTarget != target) {
                            ...
                            ModNetwork.sendToALL(level.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, vec31, 8));
                        }
                    }
        ...
        public void onCraft(ServerLevel serverLevel) {
            ...
            for (Ingredient ingredient : recipe.getIngredients()) {
                ...
                for (int i = 0; i < inv.getContainerSize(); i++) {
                    ...
                    if (!slot.isEmpty() && ingredient.test(slot)) {
                        ItemStack remainder = slot.getRecipeRemainder() != null ? slot.getRecipeRemainder() : ItemStack.EMPTY;
                        slot.shrink(1);
                        if (slot.isEmpty()) {
    ...
    public static class ForgeOminousSaddleGoal<T extends CrusherServant> extends MobSingleCraftingGoal<T> {
        ...
        public ForgeOminousSaddleGoal(T mob) {
            super(mob, 60, 0.75F, predicate(mob), itemStack -> itemStack.is(ModItems.OMINOUS_SADDLE));
        }
    ...
    public static class ForgePuttyGoal<T extends CrusherServant> extends MobSingleCraftingGoal<T> {
        ...
        public ForgePuttyGoal(T mob) {
            super(mob, 60, 0.75F, predicate(mob), itemStack -> itemStack.is(ModItems.REPAIR_PUTTY));
        }
    ...
    public static class EquipSaddleGoal extends Goal {
        ...
        private ItemStack findSaddle() {
            return this.crusher.itemsInInv(stack -> stack.is(ModItems.OMINOUS_SADDLE)).stream().findFirst().orElse(null);
        }
    ...
    public static class RepairGolemGoal extends Goal {
        ...
        private ItemStack findPutty() {
            return this.crusher.itemsInInv(stack -> stack.is(ModItems.REPAIR_PUTTY)).stream().findFirst().orElse(null);
        }
            ...
            if (!this.crusher.isWithinDistance(ally, ally.getBbWidth() + 1.0F)) {
                ...
                if (ally.level instanceof ServerLevel serverLevel) {
                    for (int i = 0; i < 7; ++i) {
                        ...
                        serverLevel.sendParticles(ModParticleTypes.HEAL_EFFECT, ally.getRandomX(1.0D), ally.getRandomY() + 0.5D, ally.getRandomZ(1.0D), 0, d0, d1, d2, 0.5F);
                    }
                }
                this.crusher.getInventory().setChanged();
            }
    ...
    public static class RepairRoyalGuardShield extends ThrowItemGoal {
        ...
        public RepairRoyalGuardShield(AbstractIllagerServant illager){
            super(illager);
            this.predicate = itemStack -> itemStack.is(ConventionalItemTags.IRON_INGOTS);
                            ...
                            if (this.throwTime > 20){
                                ...
                                if (this.target.level instanceof ServerLevel serverLevel) {
                                    for (int i = 0; i < 7; ++i) {
                                        ...
                                        serverLevel.sendParticles(ModParticleTypes.HEAL_EFFECT, this.target.getRandomX(1.0D), this.target.getRandomY() + 0.5D, this.target.getRandomZ(1.0D), 0, d0, d1, d2, 0.5F);
                                    }
                                }
                            }
                        } else {
    ...
    public static class LootSaddleGoal<T extends AbstractIllagerServant> extends IllagerChestGoal<T> {
        ...
        public LootSaddleGoal(T illager) {
            super(illager);
            this.chestPredicate = itemStack -> itemStack.is(ModItems.OMINOUS_SADDLE);
        }
        ...
        public boolean canUse() {
            ...
            if (!this.illager.itemsInInv(stack -> stack.is(ModItems.OMINOUS_SADDLE)).isEmpty()) {
                return false;
            }
            return super.canUse();
        }
    ...
    public static class LootPuttyGoal<T extends AbstractIllagerServant> extends IllagerChestGoal<T> {
        ...
        public LootPuttyGoal(T illager) {
            super(illager);
            this.chestPredicate = itemStack -> itemStack.is(ModItems.REPAIR_PUTTY);
        }
        ...
        public boolean canUse() {
            ...
            int getPutties = this.illager.itemsInInv(stack -> stack.is(ModItems.REPAIR_PUTTY)).size();
            if (getPutties >= 24) {
                return false;
            }
            return super.canUse();
