...
public class EvokerServant extends SpellcasterIllagerServant {
            ...
            if (this.getIdol() == null) {
                if (this.getTrueOwner() != null) {
                    if (CuriosFinder.hasNamelessSet(this.getTrueOwner())) {
                        BoundEvoker servant = this.convertTo(ModEntityType.BOUND_EVOKER, true);
                        if (servant != null) {
    ...
    public boolean validLootToStore(ItemStack itemStack) {
        return super.validLootToStore(itemStack) && !itemStack.is(ModItems.OMINOUS_SADDLE);
    }
    ...
    class EvokerSummonSpellGoal extends SpellcasterUseSpellGoal {
        ...
        protected void performSpellCasting() {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                AllyVex vex = ModEntityType.VEX_SERVANT.create(EvokerServant.this.level());
                if (vex != null) {
                    vex.moveTo(blockpos, 0.0F, 0.0F);
                    vex.finalizeSpawn(serverlevel, EvokerServant.this.level.getCurrentDifficultyAt(blockpos), MobSpawnType.MOB_SUMMONED, null, null);
