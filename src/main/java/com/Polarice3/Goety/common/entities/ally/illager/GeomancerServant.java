...
public class GeomancerServant extends SpellcasterIllagerServant {
            ...
            if (this.getIdol() == null) {
                if (this.getTrueOwner() != null) {
                    if (CuriosFinder.hasNamelessSet(this.getTrueOwner())) {
                        BoundGeomancer servant = this.convertTo(ModEntityType.BOUND_GEOMANCER, true);
                        if (servant != null) {
    ...
    protected SoundEvent getDeathSound() {
        return ModSounds.GEOMANCER_DEATH;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource p_32654_) {
        return ModSounds.GEOMANCER_HURT;
    }
    ...
    public static class GeomancyGoal extends Goal {
        ...
        public void start() {
            super.start();
            this.spellTick = 0;
            this.geomancer.playSound(ModSounds.GEOMANCER_PRE_ATTACK, 0.55F, 0.8F);
        ...
        public void tick() {
            ...
            if (target != null) {
                ...
                if (this.spellTick == 13) {
                    this.geomancer.playSound(ModSounds.GEOMANCER_ATTACK, 0.55F, 0.8F);
                    ...
                    if (this.geomancer.random.nextBoolean()) {
                        if (this.geomancer.random.nextFloat() <= chance) {
                            WandUtil.summonQuadOffensiveTrap(this.geomancer, target, ModEntityType.TOTEMIC_BOMB, 0);
                            ...
                            WandUtil.summonMonolith(this.geomancer, target, ModEntityType.TOTEMIC_BOMB, xShift, zShift, 0);
                        ...
                        if (random == 0) {
                            ...
                            WandUtil.summonSquareTrap(this.geomancer, target, ModEntityType.TOTEMIC_WALL, rowToRemove, 0);
                        } else if (random == 1) {
                            WandUtil.summonWallTrap(this.geomancer, target, ModEntityType.TOTEMIC_WALL, 0);
                        } else {
                            WandUtil.summonRandomPillarsTrap(this.geomancer, target, ModEntityType.TOTEMIC_WALL, 0);
                        }
                    }
