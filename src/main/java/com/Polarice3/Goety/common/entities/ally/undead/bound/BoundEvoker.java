...
public class BoundEvoker extends AbstractBoundIllager {
    ...
    public void die(DamageSource pCause) {
        this.playSound(ModSounds.DEAD_MOAN, 2.0F, 1.0F);
        super.die(pCause);
    }
    ...
    class EvokerSummonSpellGoal extends BoundUseSpellGoal {
        ...
        protected void performSpellCasting() {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                AllyVex vex = ModEntityType.VEX_SERVANT.create(BoundEvoker.this.level);
                if (vex != null) {
                    vex.moveTo(blockpos, 0.0F, 0.0F);
                    vex.finalizeSpawn(serverlevel, BoundEvoker.this.level.getCurrentDifficultyAt(blockpos), MobSpawnType.MOB_SUMMONED, null, null);
    ...
    public class EvokerWololoSpellGoal extends BoundUseSpellGoal {
        ...
        public boolean canUse() {
            if (BoundEvoker.this.getTarget() != null) {
                ...
                return false;
            } else if (!BoundEvoker.this.level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
                return false;
            } else {
