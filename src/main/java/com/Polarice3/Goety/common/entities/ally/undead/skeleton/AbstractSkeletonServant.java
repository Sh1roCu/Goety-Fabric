...
public abstract class AbstractSkeletonServant extends Summoned implements RangedAttackMob {
    ...
    public EntityType<?> getVariant(@Nullable Player player, Level level, BlockPos blockPos) {
        EntityType<?> entityType = ModEntityType.SKELETON_SERVANT;
        if (level instanceof ServerLevel serverLevel) {
            ...
            if (biome.is(ConventionalBiomeTags.CLIMATE_COLD) && biome.is(ConventionalBiomeTags.IN_OVERWORLD) && level.canSeeSky(blockPos)) {
                ...
                entityType = ModEntityType.MOSSY_SKELETON_SERVANT;
            } else if (level.isWaterAt(blockPos)) {
                entityType = ModEntityType.SUNKEN_SKELETON_SERVANT;
    ...
    protected AbstractArrow getMobArrow(ItemStack arrowStack, float distanceFactor) {
        AbstractArrow abstractarrowentity = net.minecraft.world.entity.projectile.ProjectileUtil.getMobArrow(this, arrowStack, distanceFactor);
        if (this.isUpgraded()) {
