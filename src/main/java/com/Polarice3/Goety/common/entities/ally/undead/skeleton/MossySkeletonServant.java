...
public class MossySkeletonServant extends AbstractSkeletonServant {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.MOSSY_SKELETON_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.MOSSY_SKELETON_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.MOSSY_SKELETON_DEATH;
    }

    @Override
    protected SoundEvent getStepSound() {
        return ModSounds.MOSSY_SKELETON_STEP;
    }
    ...
    public void tick() {
        ...
        if (this.level.isClientSide) {
            if (this.tickCount % 5 == 0 && this.level.random.nextBoolean()) {
                ...
                this.level.addParticle(ModParticleTypes.BIG_CULT_SPELL, this.getX(), this.getY() + 1.0D, this.getZ(),
                        colors[0],
                        colors[1],
                        colors[2]);
            }
    ...
    public SoundEvent getShootSound() {
        return ModSounds.MOSSY_SKELETON_SHOOT;
    }
