...
public class RattledServant extends AbstractSkeletonServant {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.RATTLED_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.RATTLED_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.RATTLED_DEATH;
    }

    @Override
    protected SoundEvent getStepSound() {
        return ModSounds.RATTLED_STEP;
    }
    ...
    public void tick() {
        ...
        if (this.level instanceof ServerLevel serverLevel) {
            if (this.tickCount % 5 == 0 && this.level.getRandom().nextBoolean()) {
                ServerParticleUtil.addParticlesAroundMiddleSelf(serverLevel, ModParticleTypes.BIG_ELECTRIC, this);
            }
        }
    }
    ...
    public boolean canBeAffected(MobEffectInstance pPotioneffect) {
        return super.canBeAffected(pPotioneffect)
                && pPotioneffect.getEffect() != GoetyEffects.SPASMS;
    }
}
