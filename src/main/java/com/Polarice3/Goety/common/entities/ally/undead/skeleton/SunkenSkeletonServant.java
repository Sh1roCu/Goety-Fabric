...
public class SunkenSkeletonServant extends AbstractSkeletonServant implements CrossbowAttackMob {
    ...
    protected SoundEvent getAmbientSound() {
        return this.isInWater() ? ModSounds.SUNKEN_SKELETON_AMBIENT : SoundEvents.SKELETON_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(@NotNull DamageSource pDamageSource) {
        return this.isInWater() ? ModSounds.SUNKEN_SKELETON_HURT : SoundEvents.SKELETON_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return this.isInWater() ? ModSounds.SUNKEN_SKELETON_DEATH : SoundEvents.SKELETON_DEATH;
    }

    @Override
    protected SoundEvent getStepSound() {
        return this.isInWater() ? ModSounds.SUNKEN_SKELETON_STEP : SoundEvents.SKELETON_STEP;
    }
    ...
    public void performRangedAttack(@NotNull LivingEntity pTarget, float pDistanceFactor) {
        if (this.isHolding(is -> is.getItem() instanceof CrossbowItem)) {
            ...
            this.playSound(this.isInWater() ? ModSounds.SUNKEN_SKELETON_SHOOT : SoundEvents.SKELETON_SHOOT, 1.0F, 1.0F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
    ...
    public void performCrossbowAttack(@NotNull LivingEntity shooter, float velocity) {
        ...
        if (shooter.isHolding(is -> is.getItem() instanceof CrossbowItem)) {
            SoundEvent soundEvent = this.isInWater() ? ModSounds.SUNKEN_SKELETON_SHOOT : SoundEvents.CROSSBOW_SHOOT;
    ...
    public void shootCrossbowProjectile(LivingEntity p_32323_, LivingEntity p_32324_, Projectile p_32325_, float p_32326_, float p_32327_) {
        ...
        p_32325_.shoot(vector3f.x(), vector3f.y(), vector3f.z(), p_32327_, (float) (14 - p_32323_.level().getDifficulty().getId() * 4));
        SoundEvent soundEvent = this.isInWater() ? ModSounds.SUNKEN_SKELETON_SHOOT : SoundEvents.CROSSBOW_SHOOT;
    ...
    public AbstractArrow getArrow(ItemStack pArrowStack, float pDistanceFactor) {
        ...
        int i = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.PIERCING, this.getItemInHand(ProjectileUtil.getWeaponHoldingHand(this, item -> item instanceof CrossbowItem)));
        if (i > 0) {
    ...
    static class SwimUpGoal extends Goal {
        ...
        public void tick() {
            if (this.skeleton.getY() < (double) (this.seaLevel - 1) && (this.skeleton.getNavigation().isDone() || this.skeleton.closeToNextPos())) {
                Vec3 vec3 = DefaultRandomPos.getPosTowards(this.skeleton, 4, 8, new Vec3(this.skeleton.getX(), this.seaLevel - 1, this.skeleton.getZ()), (float) Math.PI / 2F);
                if (vec3 == null) {
                    this.stuck = true;
                    return;
                }
