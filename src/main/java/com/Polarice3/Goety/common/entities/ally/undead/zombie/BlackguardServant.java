...
public class BlackguardServant extends ZombieServant implements IShielded {
    ...
    public void attackGoal() {
        this.goalSelector.addGoal(1, new ShieldedMeleeGoal<>(this) {
            public void playPreAttack() {
                this.mob.playSound(ModSounds.BLACKGUARD_PRE_ATTACK, this.mob.getSoundVolume() + 1.0F, this.mob.getVoicePitch());
            }

            public void playSmash() {
                this.mob.playSound(ModSounds.BLACKGUARD_SMASH, this.mob.getSoundVolume() + 1.0F, this.mob.getVoicePitch());
            }
        });
    ...
    public EntityType<?> getVariant(Level level, BlockPos blockPos) {
        return ModEntityType.BLACKGUARD_SERVANT;
    }
    ...
    public void die(DamageSource pCause) {
        this.playSound(ModSounds.DOUBLE_AXE_IMPACT_SHING, 1.5F * 0.5F * (this.getRandom().nextIntBetweenInclusive(5, 10) * 0.1F), (this.getRandom().nextBoolean() ? 0.65F : 0.6F) * (this.getRandom().nextIntBetweenInclusive(5, 7) * 0.1F));
        this.playSound(ModSounds.DIRT_SHATTER_THREE, 1.5F * 0.25F * (this.getRandom().nextIntBetweenInclusive(7, 10) * 0.1F), (this.getRandom().nextBoolean() ? 0.65F : 0.6F) * (this.getRandom().nextIntBetweenInclusive(7, 10) * 0.1F));
        this.playSound(ModSounds.HAMMER_SHIMMER_IMPACT_FOUR, 1.5F * 0.4F * (this.getRandom().nextIntBetweenInclusive(5, 10) * 0.1F), (this.getRandom().nextBoolean() ? 0.65F : 0.6F) * (this.getRandom().nextIntBetweenInclusive(5, 7) * 0.1F));
        this.playSound(ModSounds.PLATE_DROP, this.getSoundVolume(), this.getVoicePitch());
        super.die(pCause);
    }
    ...
    protected void playHurtSound(DamageSource p_21160_) {
        ...
        this.playSound(ModSounds.DOUBLE_AXE_IMPACT_SHING, 0.8F * 0.5F, this.getRandom().nextIntBetweenInclusive(5, 7) * 0.1F);
        this.playSound(ModSounds.DIRT_SHATTER_THREE, 0.8F * 0.25F * (this.getRandom().nextIntBetweenInclusive(7, 10) * 0.1F), this.getRandom().nextIntBetweenInclusive(7, 10) * 0.1F);
        this.playSound(ModSounds.HAMMER_SHIMMER_IMPACT_FOUR, 0.8F * 0.4F * (this.getRandom().nextIntBetweenInclusive(5, 10) * 0.1F), this.getRandom().nextIntBetweenInclusive(5, 7) * 0.1F);
        this.playSound(ModSounds.PLATE, this.getSoundVolume(), this.getVoicePitch());
    }

    @Override
    protected SoundEvent getStepSound() {
        return ModSounds.BLACKGUARD_STEP;
    }
