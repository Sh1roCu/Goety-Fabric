...
public class HauntedArmorStand extends ArmorStand {
    ...
    public HauntedArmorStand(Level p_31556_, double p_31557_, double p_31558_, double p_31559_) {
        this(ModEntityType.HAUNTED_ARMOR_STAND, p_31556_);
        ...
        if (itemStack.getItem() instanceof AnimationCore) {
            if (SEHelper.hasResearch(playerIn, ResearchList.HAUNTING)) {
                if (ItemHelper.isFullEquipped(this)) {
                    HauntedArmorServant hauntedArmorServant = new HauntedArmorServant(ModEntityType.HAUNTED_ARMOR_SERVANT, playerIn.level);
                    ...
                    if (playerIn.level.addFreshEntity(hauntedArmorServant)) {
                        this.playSound(ModSounds.SUMMON_SPELL);
    ...
    public void showBreakingParticles() {
        if (this.level instanceof ServerLevel) {
            ((ServerLevel) this.level).sendParticles(new BlockParticleOption(ParticleTypes.BLOCK, ModBlocks.HAUNTED_PLANKS.defaultBlockState()), this.getX(), this.getY(0.6666666666666666D), this.getZ(), 10, this.getBbWidth() / 4.0F, this.getBbHeight() / 4.0F, this.getBbWidth() / 4.0F, 0.05D);
        }
    }
    ...
    private void brokenByPlayer(DamageSource p_31647_) {
        Block.popResource(this.level, this.blockPosition(), new ItemStack(ModItems.HAUNTED_ARMOR_STAND));
    ...
    public ItemStack getPickResult() {
        return new ItemStack(ModItems.HAUNTED_ARMOR_STAND);
    }
}
