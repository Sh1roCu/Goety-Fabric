    ...
    protected void alertSpooders() {
        ...
        for (Mob mob : list) {
            if (mob.getTarget() == null && this.getLastHurtByMob() != null && !(this.getLastHurtByMob() instanceof Spider) && !MobUtil.areAllies(this.getLastHurtByMob(), this)) {
                if (mob instanceof Spider && !mob.getType().is(ModTags.EntityTypes.MINI_BOSSES) && !mob.getType().is(ConventionalEntityTypeTags.BOSSES) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(this.getLastHurtByMob())) {
