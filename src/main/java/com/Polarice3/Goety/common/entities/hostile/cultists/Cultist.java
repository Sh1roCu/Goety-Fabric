    ...
    protected void alertWitches() {
        ...
        for (Mob mob : list) {
            if (mob.getTarget() == null && this.getLastHurtByMob() != null && !(this.getLastHurtByMob() instanceof Raider) && !MobUtil.areAllies(this.getLastHurtByMob(), this)) {
                if ((mob instanceof Witch || mob instanceof Cultist) && !mob.getType().is(ConventionalEntityTypeTags.BOSSES) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(this.getLastHurtByMob())) {
