...
public class Heretic extends Cultist implements IHeretic {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.HERETIC_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource p_34154_) {
        return ModSounds.HERETIC_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.HERETIC_DEATH;
    }
    ...
    public void handleEntityEvent(byte pId) {
        if (pId == 4) {
            ...
            for (int i = 0; i < this.random.nextInt(35) + 10; ++i) {
                ...
                this.level.addParticle(ModParticleTypes.RISING_ENCHANT, this.getX() + this.random.nextGaussian() * (double) 0.13F, this.getBoundingBox().maxY + 0.5D + this.random.nextGaussian() * (double) 0.13F, this.getZ() + this.random.nextGaussian() * (double) 0.13F, colorUtil.red(), colorUtil.green(), colorUtil.blue());
            }
        } else {
    ...
    public static class ChantAtTargetGoal extends Goal {
        ...
        public void start() {
            ...
            this.heretic.playSound(ModSounds.HERETIC_CHANT, 2.0F, 0.5F);
        }
        ...
        public void tick() {
            if (this.targetPos != null) {
                if (this.heretic.level instanceof ServerLevel serverLevel) {
                    ColorUtil colorUtil = new ColorUtil(ChatFormatting.DARK_PURPLE);
                    ServerParticleUtil.circularParticles(serverLevel, ModParticleTypes.RISING_ENCHANT, this.targetPos.x, this.targetPos.y, this.targetPos.z, colorUtil.red, colorUtil.green, colorUtil.blue, 1.0F);
                }
                if (this.heretic.distanceToSqr(this.targetPos) > Mth.square(4.0D)) {
                    ...
                    if (this.castingTime == TOTAL_CAST_TIME) {
                        Summoned summon = new ZPiglinServant(ModEntityType.ZPIGLIN_SERVANT, this.heretic.level);
                        if (this.heretic.getRandom().nextFloat() <= 0.5F) {
                            summon = new Hellhound(ModEntityType.HELLHOUND, this.heretic.level);
                        } else if (this.heretic.getRandom().nextFloat() <= 0.25F) {
                            summon = new MagmaCubeServant(ModEntityType.MAGMA_CUBE_SERVANT, this.heretic.level);
                        } else if (this.heretic.getRandom().nextFloat() <= 0.05F) {
                            summon = new BlazeServant(ModEntityType.BLAZE_SERVANT, this.heretic.level);
                        }
                        ...
                        if (this.heretic.level.addFreshEntity(summon)) {
                            summon.playSound(ModSounds.SUMMON_SPELL_FIERY, 1.0F, 1.0F);
