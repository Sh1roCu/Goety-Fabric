            ...
            if (this.getTarget() != null) {
                if (!(this.getTarget() instanceof Raider)) {
                    if (this.getTarget().distanceTo(this) < 6.0F && this.coolDown <= this.totalCool / 2) {
                        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(ModItems.SNAP_FUNGUS));
                    } else {
                ...
                if (this.getMainHandItem().is(ModItems.SNAP_FUNGUS) || this.getMainHandItem().is(ModItems.BERSERK_FUNGUS)) {
                    this.setItemSlot(EquipmentSlot.MAINHAND, ItemStack.EMPTY);
                }
                boolean flag = false;
                if (!this.isInLava()) {
