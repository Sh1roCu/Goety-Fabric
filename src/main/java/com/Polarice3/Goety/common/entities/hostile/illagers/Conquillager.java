...
public class Conquillager extends HuntingIllagerEntity implements CrossbowAttackMob {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide) {
            if (this.tickCount % 20 == 0) {
                for (int i = 0; i < 8; ++i) {
                    this.level.addParticle(ModParticleTypes.PLAGUE_EFFECT, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0.0D, 0.5D, 0.0D);
                }
            }
        }
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_33282_, DifficultyInstance p_33283_, MobSpawnType p_33284_, @Nullable SpawnGroupData p_33285_, @Nullable CompoundTag p_33286_) {
        ...
        if (p_33284_ == MobSpawnType.EVENT) {
            if (p_33282_.getLevel().random.nextFloat() <= 0.25F && !this.isPassenger()) {
                Trampler trampler = new Trampler(ModEntityType.TRAMPLER, p_33282_.getLevel());
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.CONQUILLAGER_AMBIENT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.CONQUILLAGER_DEATH;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.CONQUILLAGER_HURT;
    }
    ...
    public SoundEvent getCelebrateSound() {
        return ModSounds.CONQUILLAGER_CELEBRATE;
    }
