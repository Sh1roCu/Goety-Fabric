...
public class Preacher extends HuntingIllagerEntity {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.PREACHER_AMBIENT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.PREACHER_DEATH;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.PREACHER_HURT;
    }

    @Override
    protected SoundEvent getCastingSoundEvent() {
        return ModSounds.PREACHER_CAST;
    }
                ...
                if (this.preacher.healTick == 10) {
                    ...
                    if (!this.preacher.level.isClientSide) {
                        ...
                        for (int i = 0; i < serverWorld.random.nextInt(10) + 10; ++i) {
                            serverWorld.sendParticles(ModParticleTypes.HEAL_EFFECT_2, this.target.getRandomX(1.5D), this.target.getRandomY(), this.target.getRandomZ(1.5D), 0, 0.0F, 1.0F, 0.0F, 1.0F);
                        }
                        this.target.playSound(ModSounds.HEAL_SPELL, this.preacher.getSoundVolume(), this.preacher.getVoicePitch());
                    }
