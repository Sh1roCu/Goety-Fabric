...
public class Malghast extends SummonedFlying {
    ...
    public void dismiss() {
        ...
        this.playSound(ModSounds.GHAST_DISAPPEAR, this.getSoundVolume(), this.getVoicePitch());
        this.discard();
    }
        ...
        public void tick() {
            ...
            if (livingentity != null && livingentity.distanceToSqr(this.ghast) < Mth.square(d0) && this.ghast.hasLineOfSight(livingentity)) {
                ...
                if (this.chargeTime == 20) {
                    ...
                    if (!this.ghast.isSilent()) {
                        if (this.shotTimes) {
                            this.ghast.playSound(ModSounds.HELL_BLAST_SHOOT, 5.0F, (this.ghast.random.nextFloat() - this.ghast.random.nextFloat()) * 0.2F + 1.0F);
                        } else {
