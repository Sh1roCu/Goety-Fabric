...
public class AbstractBorderWraith extends AbstractWraith implements IBreathing {
    ...
    public void startBreathing() {
        if (!this.isBreathing()) {
            ...
            if (!this.isSilent()) {
                this.level.playSound(null, this.getX(), this.getY(), this.getZ(), ModSounds.WRAITH_PUKE, this.getSoundSource(), 1.0F, 1.0F);
                this.playSound(ModSounds.WRAITH_PUKE, 1.0F, 1.0F);
            }
        }
    }
    ...
    public void handleEntityEvent(byte pId) {
        ...
        if (pId == 10) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                this.level.addParticle(ModParticleTypes.WRAITH_FIRE, px, py, pz, dx, dy, dz);
            }
            this.gameEvent(GameEvent.PROJECTILE_SHOOT);
        }
