...
public class AbstractMossyNecromancer extends AbstractNecromancer {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.MOSSY_NECROMANCER_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(@NotNull DamageSource damageSourceIn) {
        return ModSounds.MOSSY_NECROMANCER_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.MOSSY_NECROMANCER_DEATH;
    }

    @Override
    protected SoundEvent getStepSound() {
        return ModSounds.MOSSY_NECROMANCER_STEP;
    }

    @Nullable
    @Override
    public SoundEvent getLaughSound() {
        return ModSounds.MOSSY_NECROMANCER_LAUGH;
    }

    @Override
    public void performRangedAttack(@NotNull LivingEntity p_33317_, float p_33318_) {
        if (this.getNecroLevel() <= 0) {
            new SoulBoltSpell().mobSpellResult(this, ModItems.WILD_STAFF.getDefaultInstance());
        } else {
    ...
    public Summoned getDefaultSummon() {
        return new JungleZombieServant(ModEntityType.JUNGLE_ZOMBIE_SERVANT, this.level);
    }
    ...
    public Summoned getSummon() {
        ...
        if (this.getSummonList().stream().anyMatch(entityType -> entityType.is(ModTags.EntityTypes.ZOMBIE_SERVANTS))) {
            if (this.level.random.nextBoolean()) {
                summoned = new JungleZombieServant(ModEntityType.JUNGLE_ZOMBIE_SERVANT, this.level);
            }
        }
        if (this.getSummonList().stream().anyMatch(entityType -> entityType.is(ModTags.EntityTypes.SKELETON_SERVANTS))) {
            if (this.level.random.nextBoolean()) {
                summoned = new MossySkeletonServant(ModEntityType.MOSSY_SKELETON_SERVANT, this.level);
            }
        }
        if (this.getSummonList().contains(ModEntityType.WRAITH_SERVANT)) {
            if (this.level.random.nextFloat() <= 0.05F) {
                summoned = new MuckWraithServant(ModEntityType.MUCK_WRAITH_SERVANT, this.level);
            }
        }
