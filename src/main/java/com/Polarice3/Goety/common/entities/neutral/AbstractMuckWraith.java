...
public class AbstractMuckWraith extends AbstractWraith {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.TOWER_WRAITH_AMBIENT;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
        return ModSounds.TOWER_WRAITH_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.TOWER_WRAITH_DEATH;
    }

    @Override
    protected SoundEvent getStepSound() {
        return ModSounds.TOWER_WRAITH_FLY;
    }

    @Override
    protected SoundEvent getTeleportInSound() {
        return ModSounds.TOWER_WRAITH_TELEPORT_IN;
    }

    @Override
    protected SoundEvent getTeleportOutSound() {
        return ModSounds.TOWER_WRAITH_TELEPORT_OUT;
    }

    @Override
    protected SoundEvent getAttackSound() {
        return ModSounds.TOWER_WRAITH_ATTACK;
    }
    ...
    public void magicFire(LivingEntity livingEntity) {
        AcidPool acidPool = new AcidPool(ModEntityType.ACID_POOL, this.level);
        ...
        acidPool.setSoundEvent(ModSounds.TOWER_WRAITH_ACID_VOCAL.getLocation().toString());
        if (!livingEntity.isInWater()) {
            MobUtil.moveDownToGround(acidPool);
        }
        if (this.level.addFreshEntity(acidPool)) {
            acidPool.playSound(ModSounds.TOWER_WRAITH_ACID, 1.0F, this.getVoicePitch());
        }
    }
    ...
    public void handleEntityEvent(byte pId) {
        if (pId == 4) {
            ...
            for (int j = 0; j < 4; ++j) {
                ...
                this.level.addParticle(ModParticleTypes.BIG_CULT_SPELL, d1, d2, d3, colorUtil.red(), colorUtil.green(), colorUtil.blue());
            }
        } else {
