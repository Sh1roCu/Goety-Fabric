...
public abstract class AbstractObsidianMonolith extends AbstractMonolith {
            ...
            if (!this.isActivate()) {
                ...
                if (this.empowered > 0) {
                    ...
                    if (this.tickCount % MathHelper.secondsToTicks(20) == 0) {
                        this.playSound(ModSounds.SCARY_RECITE, 1.0F, this.getVoicePitch());
                    }
                }
                        ...
                        if (!this.level.isClientSide) {
                            ...
                            for (int p = 0; p < r.nextInt(numbers) + 1; ++p) {
                                ...
                                if (this.level.random.nextFloat() > 0.25F || !apostle.isSecondPhase()) {
                                    summon = new ZPiglinServant(ModEntityType.ZPIGLIN_SERVANT, this.level);
                                } else {
                                    summon = new ZPiglinBruteServant(ModEntityType.ZPIGLIN_BRUTE_SERVANT, this.level);
                                }
            ...
            if (MobsConfig.ObsidianMonolithSpawner.get()) {
                ...
                if (this.tickCount % 20 == 0) {
                    ...
                    if (this.spawnMiniBossCool <= 0 && j == 0 && heresiarchs == 0 && random1.nextFloat() <= 0.05F) {
                        Heresiarch heresiarch = new Heresiarch(ModEntityType.HERESIARCH, this.level);
    ...
    public void summonMavericks() {
        if (this.level instanceof ServerLevel serverLevel) {
            Maverick maverick = new Maverick(ModEntityType.MAVERICK, this.level);
    ...
    public void summonHeretics() {
        if (this.level instanceof ServerLevel serverLevel) {
            Heretic heretic = new Heretic(ModEntityType.HERETIC, this.level);
            int i1 = this.blockPosition().getX() + (Mth.randomBetweenInclusive(serverLevel.getRandom(), 4, 12) * Mth.randomBetweenInclusive(serverLevel.getRandom(), -1, 1));
            int j1 = this.blockPosition().getY() + (Mth.randomBetweenInclusive(serverLevel.getRandom(), 0, 3) * Mth.randomBetweenInclusive(serverLevel.getRandom(), -1, 1));
            int k1 = this.blockPosition().getZ() + (Mth.randomBetweenInclusive(serverLevel.getRandom(), 4, 12) * Mth.randomBetweenInclusive(serverLevel.getRandom(), -1, 1));
                ...
                for (int x1 = -3; x1 <= 3; ++x1) {
                    for (int y1 = 0; y1 <= 4; ++y1) {
                        for (int z1 = -3; z1 <= 3; ++z1) {
                            ...
                            if (!blockstate.is(BlockTags.WITHER_IMMUNE)/* && EventHooks.onEntityDestroyBlock(this, blockpos, blockstate)*/) {
            ...
            if (this.level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) && MobsConfig.ObsidianMonolithSpread.get() && this.level.dimension() != Level.NETHER) {
                if (this.startSpreading()) {
                    if (this.level.isEmptyBlock(BlockPos.containing(skyVec))) {
                        ServerParticleUtil.addAuraParticles(serverLevel, ModParticleTypes.SPELL_CLOUD, skyVec, 2.0F);
                        ServerParticleUtil.addReverseAuraParticles(serverLevel, ModParticleTypes.SPELL_CLOUD, skyVec, 6.0F);
                        ServerParticleUtil.addAuraParticles(serverLevel, ModParticleTypes.SPELL_CLOUD, skyVec, 10.0F);
                    }
                    ...
                    if (this.startUp > 0) {
                        if (this.level.isEmptyBlock(BlockPos.containing(skyVec))) {
                            ...
                            ServerParticleUtil.addReverseVisualAuraParticles(serverLevel, ModParticleTypes.SPELL_CLOUD, skyVec, radius / 2.0F);
                            ServerParticleUtil.addVisibleAuraParticles(serverLevel, ModParticleTypes.SPELL_CLOUD, skyVec, radius);
                        }
                    }
                }
            }
