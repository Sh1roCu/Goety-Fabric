...
public class AbstractWraith extends Summoned {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.WRAITH_AMBIENT;
    }
    ...
    protected SoundEvent getDeathSound() {
        return ModSounds.WRAITH_DEATH;
    }

    protected SoundEvent getStepSound() {
        return ModSounds.WRAITH_FLY;
    }

    protected SoundEvent getAttackSound() {
        return ModSounds.WRAITH_ATTACK;
    }

    protected SoundEvent getTeleportInSound() {
        return ModSounds.WRAITH_TELEPORT;
    }
    ...
    public boolean canTeleport() {
        var event = EventHooks.onEnderTeleport(this, this.getX(), this.getY(), this.getZ());
    ...
    public ParticleOptions getFireParticles() {
        return ModParticleTypes.WRAITH;
    }

    public ParticleOptions getBurstParticles() {
        return ModParticleTypes.WRAITH_BURST;
    }
    ...
    public void handleEntityEvent(byte pId) {
        ...
        if (pId == 102) {
            ...
            this.playSound(this.getAmbientSound() != null ? this.getAmbientSound() : ModSounds.WRAITH_AMBIENT, 1.0F, 2.0F);
    ...
    public EntityType<?> getVariant(@Nullable Player player, Level level, BlockPos blockPos) {
        ...
        if (this.isHostile()) {
            entityType = ModEntityType.WRAITH;
        } else {
            entityType = ModEntityType.WRAITH_SERVANT;
        }
        if (level instanceof ServerLevel serverLevel) {
            if (BlockFinder.findStructure(serverLevel, blockPos, ModTags.Structures.CAN_SUMMON_BORDER_WRAITHS)) {
                if (this.isHostile()) {
                    entityType = ModEntityType.BORDER_WRAITH;
                } else {
                    entityType = ModEntityType.BORDER_WRAITH_SERVANT;
                }
            } else if (level.getBiome(blockPos).is(ConventionalBiomeTags.SWAMP)) {
                if (this.isHostile()) {
                    entityType = ModEntityType.MUCK_WRAITH;
                } else {
                    entityType = ModEntityType.MUCK_WRAITH_SERVANT;
                }
            }
        }
        return entityType;
    ...
    public InteractionResult mobInteract(Player pPlayer, InteractionHand pHand) {
        ...
        if (this.getTrueOwner() != null && pPlayer == this.getTrueOwner()) {
            if (itemstack.is(ModItems.ECTOPLASM) && this.getHealth() < this.getMaxHealth()) {
                ...
                this.playSound(ModSounds.WRAITH_AMBIENT, 1.0F, 1.25F);
                this.heal(2.0F);
                if (this.level instanceof ServerLevel serverLevel) {
                    for (int i = 0; i < 7; ++i) {
                        double d0 = this.random.nextGaussian() * 0.02D;
                ...
                if (!actionresulttype.consumesAction()) {
                    ...
                    this.playSound(ModSounds.WRAITH_AMBIENT, 1.0F, 2.0F);
                    this.heal(1.0F);
                    return InteractionResult.SUCCESS;
                }
