...
public class BurningHoglin extends Summoned implements ICharger, HoglinBase {
    ...
    public SoundEvent getAmbientSound() {
        return ModSounds.MCD_HOGLIN_AMBIENT;
    }

    @Override
    public SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.MCD_HOGLIN_HURT;
    }

    @Override
    public SoundEvent getDeathSound() {
        return ModSounds.MCD_HOGLIN_DEATH;
    }

    protected SoundEvent getStepSound() {
        return ModSounds.MCD_HOGLIN_STEP;
    }

    @Override
    protected void playStepSound(BlockPos p_180429_1_, BlockState p_180429_2_) {
        if (this.isCharging()) {
            this.playSound(ModSounds.MCD_HOGLIN_GALLOP, 1.0F, 1.0F);
        } else {
    ...
    public void aiStep() {
        ...
        if (this.level instanceof ServerLevel serverLevel) {
            ...
            serverLevel.sendParticles(ModParticleTypes.SMALL_FIRE, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0, d0, d1, d2, 1.0F);
        }
    }
        ...
        if (!this.level.isClientSide) {
            if (this.level instanceof ServerLevel serverLevel) {
                ...
                for (int i = 0; i < 32; ++i) {
                    ...
                    serverLevel.sendParticles(ModParticleTypes.BIG_CULT_SPELL, this.getRandomX(1.0F), this.getRandomY(), this.getRandomZ(1.0F), 0, colorUtil1.red, colorUtil1.green, colorUtil1.blue, 1.0F);
                    serverLevel.sendParticles(ModParticleTypes.BIG_FIRE_GROUND, this.getRandomX(0.5F), this.getY(), this.getRandomZ(0.5F), 1, 0.0F, 0.0F, 0.0F, 0.0F);
                }
            }
            this.dead = true;
    ...
    public static class BHChargeGoal extends ChargeGoal {
        ...
        public void start() {
            this.chargeTick = 0;
            this.hoglin.playSound(ModSounds.MCD_HOGLIN_GROWL);
        ...
        public void tick() {
            ...
            if (this.windup > 0) {
                ...
                if (this.windup == 1) {
                    this.hoglin.playSound(ModSounds.MCD_HOGLIN_SQUEEL);
                }
