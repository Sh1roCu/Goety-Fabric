...
public class GulfTentacle extends Owned {
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            ...
            if (!this.sentSpawnEvent) {
                ...
                ModNetwork.sentToTrackingEntityAndPlayer(this, STentacleRangePacket.ID, STentacleRangePacket.encode(this.getId(), this.getRange()));
                ModNetwork.sentToTrackingEntityAndPlayer(this, SRepositionPacket.ID, SRepositionPacket.encode(this.getId(), this.position()));
            }
            if (this.life >= 20) {
                this.discard();
            } else {
    ...
    public void damageEntities(Set<LivingEntity> entities) {
        for (LivingEntity entity : entities) {
            ...
            if (flag) {
                this.level.playSound(null, entity.getX(), entity.getY(), entity.getZ(), ModSounds.WHIP_HIT, this.getSoundSource(), 1.0F, this.getVoicePitch());
                if (this.isStaff) {
                    GulfTentacle gulfTentacle = new GulfTentacle(ModEntityType.GULF_TENTACLE, this.level);
                    ...
                    if (this.hasEffect(GoetyEffects.BUFF)) {
                        gulfTentacle.addEffect(new MobEffectInstance(GoetyEffects.BUFF, EffectsUtil.infiniteEffect(), EffectsUtil.getAmplifier(this, GoetyEffects.BUFF), false, false));
                    }
                    gulfTentacle.setStaff(false);
                    this.level.addFreshEntity(gulfTentacle);
                    gulfTentacle.playSound(ModSounds.WHIP_SWING);
                }
            }
        }
    }
