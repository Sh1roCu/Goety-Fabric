...
public class SpiderEgg extends Owned {
    ...
    protected SoundEvent getHurtSound(DamageSource p_21239_) {
        return ModSounds.SPIDER_NEST_START;
    }
    ...
    public void tick() {
        ...
        if (i > 0 && this.swell == 0) {
            this.playSound(ModSounds.SPIDER_NEST_START, 0.5F, 0.75F);
        }
        this.swell += i;
        if (this.swell < 0) {
            this.swell = 0;
    ...
    private void hatchEgg() {
        if (this.level instanceof ServerLevel serverLevel) {
            SpiderServant spiderServant = ModEntityType.SPIDER_SERVANT.create(serverLevel);
            if (spiderServant != null) {
                ...
                if (spiderServant != null) {
                    ...
                    if (serverLevel.addFreshEntity(spiderServant)) {
                        this.playSound(ModSounds.SPIDER_NEST_SPAWN);
