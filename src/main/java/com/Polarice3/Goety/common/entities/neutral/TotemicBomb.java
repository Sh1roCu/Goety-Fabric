...
public class TotemicBomb extends AbstractMonolith {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        ...
        if (this.canSpawn(pLevel.getLevel())) {
            this.playSound(ModSounds.BOMB_SPAWN, 2.0F, 1.0F);
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (!pSource.is(DamageTypeTags.BYPASSES_ARMOR)) {
            this.playSound(ModSounds.BOMB_SPARKLE);
        }
    ...
    public void aiStep() {
        ...
        if (!this.isEmerging()) {
            if (!this.isActivate()) {
                ...
                this.playSound(ModSounds.BOMB_LOAD, 2.0F, 1.0F);
            }
            if (this.tickCount == MathHelper.secondsToTicks(1)) {
                this.playSound(ModSounds.BOMB_FUSE, 2.0F, 1.0F);
            }
            if (this.tickCount == MathHelper.secondsToTicks(this.getLifeSpan() - 2)) {
                this.playSound(ModSounds.BOMB_PULSE, 2.0F, 1.0F);
            }
