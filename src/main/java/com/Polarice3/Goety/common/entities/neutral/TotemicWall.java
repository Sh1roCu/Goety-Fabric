...
public class TotemicWall extends AbstractMonolith {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData, @Nullable CompoundTag pDataTag) {
        ...
        if (this.canSpawn(pLevel.getLevel())) {
            this.playSound(ModSounds.WALL_SPAWN, 2.0F, 1.0F);
            this.playSound(ModSounds.WALL_ERUPT, 1.0F, 1.0F);
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (!pSource.is(DamageTypeTags.BYPASSES_ARMOR)) {
            this.playSound(ModSounds.WALL_HIT);
        }
    ...
    public void aiStep() {
        ...
        if (!this.isEmerging()) {
            ...
            if (this.tickCount == MathHelper.secondsToTicks(this.getLifeSpan())) {
                this.playSound(ModSounds.WALL_DISAPPEAR, 2.0F, 1.0F);
            }
        }
    }
}
