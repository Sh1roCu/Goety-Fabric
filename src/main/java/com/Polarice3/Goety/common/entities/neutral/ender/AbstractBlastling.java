...
public class AbstractBlastling extends AbstractEnderling implements RangedAttackMob {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.BLASTLING_AMBIENT;
    }
    ...
    public void stepSound() {
        this.playSound(ModSounds.BLASTLING_STEP, 0.15F, 1.0F);
    }
        ...
        if (this.level.isClientSide) {
            if (!this.isHiding()) {
                ...
                if (this.tickCount % 2 == 0) {
                    ...
                    this.level.addParticle(ModParticleTypes.SMALL_STATION_CULT_SPELL, this.getX() + (vec3.x / 2.0D), this.getEyeY() + 0.35F, this.getZ() + (vec3.z / 2.0D), colorUtil.red(), colorUtil.green(), colorUtil.blue());
                }
            }
    ...
    private void shoot(boolean leftArm, LivingEntity target) {
        ...
        this.playSound(ModSounds.BLASTLING_SHOOT, 1.0F, this.getVoicePitch());
    }
    ...
    public static class BlastlingRangedGoal extends Goal {
        ...
        public void start() {
            ...
            this.mob.playSound(ModSounds.BLASTLING_PRE_ATTACK);
        }
