    ...
    public void cloudParticles(int color) {
        if (this.level instanceof ServerLevel serverWorld) {
            ...
            for (int k1 = 0; (float) k1 < f5; ++k1) {
                ...
                if (color == 0xffffff) {
                    serverWorld.sendParticles(ModParticleTypes.SPELL_CLOUD, this.getX() + (double) f8, this.getY(), this.getZ() + (double) f9, 0, 1.0F, 1.0F, 1.0F, 0.5F);
                } else {
                    serverWorld.sendParticles(ModParticleTypes.SPELL_CLOUD, this.getX() + (double) f8, this.getY(), this.getZ() + (double) f9, 0, MathHelper.rgbParticle(color)[0], MathHelper.rgbParticle(color)[1], MathHelper.rgbParticle(color)[2], 0.5F);
                }
            }
        }
    }
