...
public class AcidPool extends AbstractTrap {
    ...
    public AcidPool(EntityType<?> entityTypeIn, Level worldIn) {
        ...
        this.setParticle(ModParticleTypes.NONE);
        this.setDuration(50);
    }
        ...
        if (this.level instanceof ServerLevel serverLevel) {
            if (this.getWarmupDelayTicks() <= 0) {
                if (!this.sentSpikeEvent) {
                    ...
                    ServerParticleUtil.circularParticles(serverLevel, ModParticleTypes.BIG_CULT_SPELL, this.getX(), this.getY() + 1.0F, this.getZ(), colorUtil.red(), colorUtil.green(), colorUtil.blue(), this.radius());
                    ServerParticleUtil.circularParticles(serverLevel, ModParticleTypes.ROLLING_SPIRAL, this.getX(), this.getY() + 1.0F, this.getZ(), colorUtil.red(), colorUtil.green(), colorUtil.blue(), this.radius());
                ...
                if (this.level.getRandom().nextBoolean()) {
                    ...
                    ServerParticleUtil.circularParticles(serverLevel, ModParticleTypes.BIG_CULT_SPELL, this.getX(), this.getY() - 0.5F, this.getZ(), colorUtil.red(), colorUtil.green(), colorUtil.blue(), this.radius() / 2.0F);
                }
            }

            if (this.sentSpikeEvent) {
                ++this.lifeTick;
                if (this.getParticle() == null || this.getParticle() == ModParticleTypes.NONE) {
