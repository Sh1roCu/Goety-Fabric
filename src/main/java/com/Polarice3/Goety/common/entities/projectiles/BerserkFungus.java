...
public class BerserkFungus extends ThrowableFungus {
    ...
    public BerserkFungus(double p_37457_, double p_37458_, double p_37459_, Level p_37460_) {
        super(ModEntityType.BERSERK_FUNGUS, p_37457_, p_37458_, p_37459_, p_37460_);
    }

    public BerserkFungus(LivingEntity p_37463_, Level p_37464_) {
        super(ModEntityType.BERSERK_FUNGUS, p_37463_, p_37464_);
    }
    ...
    protected void onHit(HitResult p_37406_) {
        ...
        if (!this.level.isClientSide) {
            this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.BLAST_FUNGUS_EXPLODE, SoundSource.BLOCKS, 1.0F, (1.0F + (this.level.random.nextFloat() - this.level.random.nextFloat()) * 0.2F) * 0.7F, false);
    ...
    public void handleEntityEvent(byte p_34138_) {
        if (p_34138_ == 15) {
            ...
            for (int k1 = 0; (float) k1 < 100; ++k1) {
                ...
                this.level.addParticle(ModParticleTypes.CULT_SPELL, this.getX() + (double) f8, this.getY(), this.getZ() + (double) f9, MathHelper.rgbToSpeed(162), MathHelper.rgbToSpeed(28), 0.0D);
            }
            this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.BLAST_FUNGUS_EXPLODE, SoundSource.BLOCKS, 1.0F, (2.0F + (this.level.random.nextFloat() - this.level.random.nextFloat()) * 0.2F) * 0.7F, false);
        } else {
