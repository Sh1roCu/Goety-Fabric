...
public class BlossomBall extends SpellThrowableProjectile {
    ...
    public BlossomBall(LivingEntity pOwner, Level p_37464_) {
        super(ModEntityType.BLOSSOM_BALL, pOwner, p_37464_);
    }
    ...
    public void explode(HitResult pResult) {
        if (!this.level.isClientSide) {
            this.playSound(ModSounds.QUICK_GROWING_VINE_BURROW, 2.0F, 0.75F);
            if (this.getOwner() != null) {
                ...
                for (Vec3 point : circlePoints) {
                    ...
                    if (this.level instanceof ServerLevel serverLevel) {
                        serverLevel.sendParticles(ModParticleTypes.BLOSSOM_THORN_INDICATOR, blossomThorn.position().x, blossomThorn.position().y + 0.1F, blossomThorn.position().z, 1, 0.0F, 0.0F, 0.0F, 0.0F);
                    }
