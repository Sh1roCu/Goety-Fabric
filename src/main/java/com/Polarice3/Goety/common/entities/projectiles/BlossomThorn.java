...
public class BlossomThorn extends GroundProjectile {
    ...
    public BlossomThorn(Level world, double pPosX, double pPosY, double pPosZ, int pWarmUp, LivingEntity owner) {
        this(ModEntityType.BLOSSOM_THORN, world);
    ...
    public void handleEntityEvent(byte pId) {
        ...
        if (pId == 5) {
            if (!this.isSilent()) {
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.VINE_TRAP_BURST, this.getSoundSource(), 1.0F, 0.75F, false);
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.QUICK_GROWING_VINE_BURST, this.getSoundSource(), 1.0F, 0.75F, false);
            }
        }
