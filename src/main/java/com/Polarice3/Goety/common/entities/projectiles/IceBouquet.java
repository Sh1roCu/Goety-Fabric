...
public class IceBouquet extends GroundProjectile {
    ...
    public IceBouquet(Level world, double pPosX, double pPosY, double pPosZ, @Nullable LivingEntity owner) {
        this(ModEntityType.ICE_BOUQUET, world);
        this.setOwner(owner);
    ...
    public void handleEntityEvent(byte pId) {
        ...
        if (pId == 5) {
            if (!this.isSilent()) {
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.WRAITH_FIRE, this.getSoundSource(), 1.0F, 1.0F, false);
            }
        }
        if (pId == 6) {
            if (!this.isSilent()) {
        ...
        if (pId == 8) {
            if (!this.isSilent()) {
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.REDSTONE_FIRE_PROJECTILE, this.getSoundSource(), 0.3F, 1.3F, false);
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.FIRE_PROJECTILE_FLY, this.getSoundSource(), 0.3F, 1.3F, false);
            }
        }
