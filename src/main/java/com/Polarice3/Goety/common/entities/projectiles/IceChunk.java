...
public class IceChunk extends SpellEntity {
    ...
    public IceChunk(Level pLevel, LivingEntity pOwner, LivingEntity pTarget) {
        this(ModEntityType.ICE_CHUNK, pLevel);
        if (pTarget != null) {
    ...
    private void onHit(HitResult hitResult) {
        if (!this.level.isClientSide()) {
            ...
            this.playSound(ModSounds.ICE_CHUNK_HIT, 2.0F, 1.0F);
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.hovering == 20) {
                for (int i = 0; i < serverWorld.random.nextInt(10) + 10; ++i) {
                    serverWorld.sendParticles(ModParticleTypes.SUMMON, this.getRandomX(1.0D), this.getRandomY(), this.getRandomZ(1.0D), 0, 0.0F, 0.0F, 0.0F, 1.0F);
                }
            }
        } else {
            ++this.hovering;
        ...
        if (this.hovering == 1 && !(this.isInWall() || this.onGround())) {
            this.playSound(ModSounds.ICE_CHUNK_IDLE, 1.0F, 1.0F);
        }
        ...
        if (this.hovering == hoverTime - 15) {
            this.playSound(ModSounds.ICE_CHUNK_DROP, 1.0F, 1.0F);
        }
        if (isHovering) {
