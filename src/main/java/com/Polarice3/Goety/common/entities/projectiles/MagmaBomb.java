...
public class MagmaBomb extends SpellThrowableProjectile {
    ...
    public MagmaBomb(double p_37457_, double p_37458_, double p_37459_, Level p_37460_) {
        super(ModEntityType.MAGMA_BOMB, p_37457_, p_37458_, p_37459_, p_37460_);
    }

    public MagmaBomb(LivingEntity pOwner, Level p_37464_) {
        super(ModEntityType.MAGMA_BOMB, pOwner, p_37464_);
    }
    ...
    public void tick() {
        ...
        this.level.addParticle(ModParticleTypes.BIG_FIRE, d0 + level.random.nextDouble() / 2, d1 + 0.5D, d2 + level.random.nextDouble() / 2, 0.0D, 0.0D, 0.0D);
    }
    ...
    public void explode(HitResult pResult) {
        if (!this.level.isClientSide) {
            ...
            if (this.level instanceof ServerLevel serverLevel) {
                ServerParticleUtil.addParticlesAroundSelf(serverLevel, ModParticleTypes.BIG_FIRE, this);
                ...
                ServerParticleUtil.circularParticles(serverLevel, ModParticleTypes.BIG_FIRE_GROUND, vec3.x, this.getY() + 0.25D, vec3.z, 0, 0, 0, this.explosionPower);
            ...
            for (int i1 = 0; i1 < i; i1++) {
                MagmaCubeServant magmaCube = ModEntityType.MAGMA_CUBE_SERVANT.create(this.level);
                if (magmaCube != null) {
