...
public class MonsoonCloud extends AbstractSpellCloud {
    ...
    public MonsoonCloud(Level pLevel, LivingEntity pOwner, LivingEntity pTarget) {
        this(ModEntityType.MONSOON_CLOUD, pLevel);
        if (pOwner != null) {
            ...
            if (livingEntity != null && !livingEntity.isDeadOrDying() && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(livingEntity)) {
                ...
                if (this.random.nextFloat() <= 0.05F) {
                    ...
                    if (lightningRod.isPresent() && !this.isStaff()) {
                        ...
                        ModNetwork.sendToALL(serverLevel.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ()), colorUtil, 10));
                        serverLevel.playSound(null, this.getX(), this.getY(), this.getZ(), ModSounds.THUNDERBOLT, this.getSoundSource(), 1.0F, 1.0F);
                    } else {
                        Vec3 vec31 = new Vec3(livingEntity.getX(), livingEntity.getY() + livingEntity.getBbHeight() / 2, livingEntity.getZ());
                        ModNetwork.sendToALL(serverLevel.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, vec31, colorUtil, 10));
