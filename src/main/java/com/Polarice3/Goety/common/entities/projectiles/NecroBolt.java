...
public class NecroBolt extends SpellHurtingProjectile {
    ...
    public NecroBolt(double pX, double pY, double pZ, double pXPower, double pYPower, double pZPower, Level pLevel) {
        super(ModEntityType.NECRO_BOLT, pX, pY, pZ, pXPower, pYPower, pZPower, pLevel);
        this.rotSpeed = 0.05F;
        this.roll = (float) Math.random() * ((float) Math.PI * 2F);
    }

    public NecroBolt(LivingEntity pShooter, double pXPower, double pYPower, double pZPower, Level pLevel) {
        super(ModEntityType.NECRO_BOLT, pShooter, pXPower, pYPower, pZPower, pLevel);
        this.rotSpeed = 0.05F;
        this.roll = (float) Math.random() * ((float) Math.PI * 2F);
    }
    ...
    protected void onHitBlock(BlockHitResult p_230299_1_) {
        ...
        if (!this.level.isClientSide) {
            ...
            for (int p = 0; p < 8; ++p) {
                ...
                serverLevel.sendParticles(ModParticleTypes.NECRO_EFFECT, d0, d1, d2, 0, 0.45, 0.45, 0.45, 0.5F);
                serverLevel.sendParticles(ModParticleTypes.SUMMON_TRAIL, d0, d1, d2, 1, 0.0F, 0.0F, 0.0F, 0);
            }
            this.discard();
        }
    }
    ...
    public void tick() {
        ...
        if (this.level.isClientSide || (entity == null || !entity.isRemoved()) && this.level.isLoaded(this.blockPosition())) {
            ...
            this.level.addParticle(ModParticleTypes.SUMMON_TRAIL,
