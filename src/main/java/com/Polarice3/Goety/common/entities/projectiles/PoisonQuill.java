...
public class PoisonQuill extends Arrow implements ISpellEntity {
    ...
    public EntityType<?> getType() {
        return ModEntityType.POISON_QUILL;
    }
    ...
    public void tick() {
        ...
        if (!this.inGround) {
            ...
            for (int i = 0; i < 4; ++i) {
                this.level.addParticle(ModParticleTypes.TRAIL, d0 + (this.random.nextGaussian() / 2), d1 + 0.5D + (this.random.nextGaussian() / 2), d2 + (this.random.nextGaussian() / 2), colorUtil.red(), colorUtil.green(), colorUtil.blue());
            }
        } else {
            this.discard();
        }
    ...
    protected SoundEvent getDefaultHitGroundSoundEvent() {
        if (this.isAqua()) {
            return ModSounds.POISON_QUILL_AQUA_IMPACT;
        }
        return ModSounds.POISON_QUILL_IMPACT;
    }
}
