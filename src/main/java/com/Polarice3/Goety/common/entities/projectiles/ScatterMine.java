...
public class ScatterMine extends Entity implements ISpellEntity {
    ...
    public ScatterMine(Level pLevel, LivingEntity pOwner, BlockPos blockPos) {
        this(ModEntityType.SCATTER_MINE, pLevel);
        if (blockPos != null) {
    ...
    public void handleEntityEvent(byte id) {
        if (id == 4) {
            this.level.addParticle(ModParticleTypes.BIG_ELECTRIC, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), -0.05D + this.random.nextDouble() * 0.05D, -0.05D + this.random.nextDouble() * 0.05D, -0.05D + this.random.nextDouble() * 0.05D);
        } else if (id == 5) {
            for (int i = 0; i < 20; ++i) {
        ...
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                for (int k = -radius; k <= radius; ++k) {
                    ...
                    this.level.addParticle(ModParticleTypes.REDSTONE_EXPLODE, d0, d1, d2, d3 / d6, d4 / d6, d5 / d6);
    ...
    public void finalizeExplosion() {
        if (this.level.isClientSide) {
            this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.REDSTONE_EXPLODE, SoundSource.BLOCKS, 4.0F, (1.0F + (this.level.random.nextFloat() - this.level.random.nextFloat()) * 0.2F) * 0.7F, false);
        }
        this.createParticleBall(0.5D, (int) 2.0F);
        this.level.addParticle(ModParticleTypes.BIG_ELECTRIC, this.getX(), this.getY(), this.getZ(), 1.0D, 0.0D, 0.0D);
    }
