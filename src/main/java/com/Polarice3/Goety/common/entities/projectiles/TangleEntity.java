...
public abstract class TangleEntity extends Entity {
    ...
    public void tangleTarget() {
        if (this.getTarget() != null
            ...
            if (this.level.isClientSide) {
                ModNetwork.sendToServer(CSetDeltaMovement.ID, CSetDeltaMovement.encode(this.getTarget().getId(), 0.0D, 0.0D, 0.0D));
            } else {
                if (this.activeTick < 10) {
                    ModNetwork.sentToTrackingEntityAndPlayer(this.getTarget(), SRepositionPacket.ID, SRepositionPacket.encode(this.getTarget().getId(), this.position().x, this.position().y, this.position().z));
                }
            }
