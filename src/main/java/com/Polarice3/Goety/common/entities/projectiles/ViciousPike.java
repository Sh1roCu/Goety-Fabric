...
public class ViciousPike extends Entity implements ISpellEntity {
    ...
    public ViciousPike(Level pLevel, LivingEntity pOwner) {
        this(ModEntityType.VICIOUS_PIKE, pLevel);
        this.owner = pOwner;
    }
    ...
    public void handleEntityEvent(byte p_36935_) {
        ...
        if (p_36935_ == 4) {
            ...
            if (!this.isSilent()) {
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.IMPALE, this.getSoundSource(), 1.0F, this.random.nextFloat() * 0.2F + 0.85F, false);
            }
        }
