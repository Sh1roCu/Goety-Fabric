...
public class ViciousTooth extends Entity implements ISpellEntity {
    ...
    public ViciousTooth(Level pLevel, LivingEntity pOwner) {
        this(ModEntityType.VICIOUS_TOOTH, pLevel);
        this.owner = pOwner;
    }
    ...
    public void tick() {
        ...
        if (!this.isDropping) {
            ...
            if (this.level instanceof ServerLevel serverLevel) {
                BlockState blockState = Blocks.TUFF.defaultBlockState();
                ServerParticleUtil.circularParticles(serverLevel, new BlockParticleOption(ModParticleTypes.FAST_DUST, blockState), this.getX(), this.getY(), this.getZ(), 0.25F);
            }
        } else {
