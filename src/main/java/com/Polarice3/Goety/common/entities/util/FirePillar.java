...
public class FirePillar extends CastSpellTrap {
    ...
    public FirePillar(Level worldIn, double x, double y, double z) {
        this(ModEntityType.FIRE_PILLAR, worldIn);
        this.setPos(x, y, z);
    }
    ...
    public ParticleOptions getParticle() {
        return ModParticleTypes.BURNING;
    }
            ...
            if (this.warmUp <= 0) {
                if (this.level instanceof ServerLevel serverWorld) {
                    if (this.tickCount % 5 == 0) {
                        serverWorld.sendParticles(ModParticleTypes.FIERY_PILLAR, this.getX(), this.getY() + 0.5D, this.getZ(), 0, 0, 0.5D, 0, 1.0D);
                    }
                }
                this.setActivated(true);
                if (!this.playEvent) {
                    this.playSound(ModSounds.FIRE_BREATH_START, 0.05F, 1.0F);
                    this.playEvent = true;
                }
                if (this.tickCount % 20 == 0) {
                    this.playSound(ModSounds.FIRE_TORNADO_AMBIENT, 1.0F, 1.0F);
                }
                ...
                if (this.level instanceof ServerLevel serverWorld) {
                    ...
                    serverWorld.sendParticles(ModParticleTypes.BIG_FIRE_GROUND, d1, d2, d3, 0, 0.0D, 0.0D, 0.0D, 0.5F);
                }
