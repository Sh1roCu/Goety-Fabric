...
public class TridentStorm extends CastSpellTrap {
    ...
    public void tick() {
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.tickTime == (time + this.delay + 5)) {
                ...
                ModNetwork.sendToALL(this.level.getServer(), SLightningBoltPacket.ID, SLightningBoltPacket.encode(vec3.add(0.0D, 250.0D, 0.0D), this.position(), 5));
                for (int i = 0; i < 16; ++i) {
                    Vec3 vec31 = vec3.add(this.random.nextDouble(), 1.0D, this.random.nextDouble());
                    ModNetwork.sendToALL(this.level.getServer(), SLightningPacket.ID, SLightningPacket.encode(vec3, vec31, 2));
                }
            }
    ...
    public void handleEntityEvent(byte p_36935_) {
        ...
        if (p_36935_ == 4) {
            ...
            if (!this.isSilent()) {
                this.level.playLocalSound(this.getX(), this.getY(), this.getZ(), ModSounds.TRIDENT_STORM_EXPLODE, this.getSoundSource(), 2.0F, this.random.nextFloat() * 0.2F + 0.85F, false);
            }
        }
