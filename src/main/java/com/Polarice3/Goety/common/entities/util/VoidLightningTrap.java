...
public class VoidLightningTrap extends MagicLightningTrap {
    ...
    public VoidLightningTrap(Level worldIn, double x, double y, double z) {
        this(ModEntityType.VOID_LIGHTNING_TRAP, worldIn);
        this.setPos(x, y, z);
    }
    ...
    public void finalizeAttack() {
        if (this.level instanceof ServerLevel serverLevel) {
            ...
            if (this.getOwner() != null) {
                ModNetwork.sendToALL(serverLevel.getServer(), SLightningBoltPacket.ID, SLightningBoltPacket.encode(this.getOwner().position().add(0, this.getOwner().getBbHeight(), 0), this.getOwner().position().add(0, 250, 0), colorUtil2, 10));
            }
            ModNetwork.sendToALL(serverLevel.getServer(), SLightningBoltPacket.ID, SLightningBoltPacket.encode(new Vec3(this.getX(), this.getY() + 250, this.getZ()), this.position(), colorUtil2, 10));
            for (int i = 0; i < 8; ++i) {
            ...
            for (int i = 0; i < 16; ++i) {
                ...
                ModNetwork.sendToALL(serverLevel.getServer(), SLightningPacket.ID, SLightningPacket.encode(vec3, vec31, colorUtil2, 12));
            }
