...
public class HauntedBroom extends Entity implements OwnableEntity {
    ...
    public HauntedBroom(ItemStack itemStack, Level level, double x, double y, double z) {
        this(ModEntityType.HAUNTED_BROOM, level);
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            ...
            if (flag || this.getDamage() > this.getDamageThreshold()) {
                this.playSound(ModSounds.BROOM_BREAK, 0.8F, 1.0F);
                if (!flag && this.level.getGameRules().getBoolean(GameRules.RULE_DOENTITYDROPS)) {
                    this.spawnBroomItem();
                }
                this.discard();
            } else {
                this.playSound(ModSounds.BROOM_SWING, 0.8F, 1.0F);
            }
            return true;
        } else {
            return true;
    ...
    private void checkCollisionDamage(double speedBeforeImpact) {
        if (this.horizontalCollision) {
            if (this.level.isClientSide) {
                ModNetwork.sendToServer(CBroomCollisionPacket.ID, CBroomCollisionPacket.encode(this.getId(), speedBeforeImpact));
            } else {
    ...
    public ItemStack getPickResult() {
        return new ItemStack(ModItems.HAUNTED_BROOM);
    }
        ...
        if (!this.getItem().isEmpty()) {
            if (this.getOwner() instanceof Player player) {
                ...
                if (!player.getInventory().add(itemStack)) {
                    ...
                    if (!this.level.isClientSide) {
                        ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.ITEM_PICKUP, 0.2F, (this.random.nextFloat() - this.random.nextFloat()) * 1.4F + 2.0F));
                    }
                }
            } else {
            ...
            this.spawnAtLocation(new ItemStack(ModItems.HAUNTED_BROOM));
        }
    }
    ...
    public boolean isBurning() {
        return EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.BURNING, this.getItem()) > 0;
    }

    public double getSpeedBoost() {
        return EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.VELOCITY, this.getItem()) / 10.0F;
    }
}
