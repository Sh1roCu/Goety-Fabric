...
import cn.sh1rocu.goety.mixin.accessor.PotionSlotAccessor;
...
//        }
        ...
        brewPotions(items, itemstack, SLOTS_FOR_SIDES);
        // EventHooks.onPotionBrewed(items);
        if (itemstack.getRecipeRemainder() != null) {
            ItemStack itemstack1 = itemstack.getRecipeRemainder();
            itemstack.shrink(1);
            if (itemstack.isEmpty()) {
    ...
    private static boolean canBrew(NonNullList<ItemStack> inputs, ItemStack ingredient, int[] inputIndexes) {
    ...
    private static boolean hasOutput(ItemStack input, ItemStack ingredient) {
        return !getOutput(input, ingredient).isEmpty();
    }

    private static void brewPotions(NonNullList<ItemStack> inputs, ItemStack ingredient, int[] inputIndexes) {
        for (int i : inputIndexes) {
    ...
    private static boolean isIngredient(ItemStack stack) {
        // Fabric: PotionBrewingMixin will check custom ingredient, it's unnecessary to call FabricCustomBrewingRegistry.isCustomIngredient
        return PotionBrewing.isIngredient(stack);
    }

    private boolean isInput(ItemStack stack) {
        // Fabric: PotionSlotMixin will check custom input, it's unnecessary to call FabricCustomBrewingRegistry.isCustomInput
        return PotionSlotAccessor.goety$mayPlaceItem(stack);
    }
        ...
        if (this.getLivingEntity() != null) {
            ...
            if (!this.getLivingEntity().level.isClientSide) {
                if (this.getLivingEntity() instanceof ServerPlayer serverPlayer) {
                    ModNetwork.sendTo(serverPlayer, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(soundEvent, 1.0F, 1.0F));
                }
            }
        }
    }
    ...
    public boolean canPlaceItem(int pIndex, ItemStack pStack) {
        if (pIndex == 3) {
            return isIngredient(pStack);
        } else {
            Item item = pStack.getItem();
            if (pIndex == 4) {
                return item == Items.BLAZE_POWDER;
            } else {
                return isInput(pStack) && this.getItem(pIndex).isEmpty();
            }
        }
    }
