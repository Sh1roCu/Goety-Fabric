...
public class BlastFungusItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!player.isSilent()) {
            level.playSound(null, player.getX(), player.getY(), player.getZ(), ModSounds.BLAST_FUNGUS_THROW, player.getSoundSource(), 2.0F, 0.8F + level.random.nextFloat() * 0.4F);
        }
        itemstack.shrink(used);
        player.getCooldowns().addCooldown(ModItems.BLAST_FUNGUS, MathHelper.secondsToTicks(random));
