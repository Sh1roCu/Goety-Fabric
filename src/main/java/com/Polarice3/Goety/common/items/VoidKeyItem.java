...
public class VoidKeyItem extends ItemBase {
    ...
    public InteractionResult useOn(UseOnContext p_41182_) {
        ...
        if (blockstate.is(ModBlocks.VOID_FRAME) && blockstate.getValue(VoidFrameBlock.LOCKED)) {
            if (level.isClientSide) {
                ...
                level.updateNeighbourForOutputSignal(blockPos, ModBlocks.VOID_FRAME);
                p_41182_.getItemInHand().shrink(1);
                serverLevel.playSound(null, blockPos.getX(), blockPos.getY(), blockPos.getZ(), ModSounds.VOID_FRAME_UNLOCK, SoundSource.BLOCKS, 2.0F, 1.0F);
                Vec3 vec3 = blockPos.above().getCenter();
                Endersent endersent = new Endersent(ModEntityType.ENDERSENT, serverLevel);
