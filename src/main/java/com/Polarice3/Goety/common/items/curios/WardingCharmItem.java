...
public class WardingCharmItem extends SingleStackItem {
                    ...
                    if (!player.hasEffect(GoetyEffects.SOUL_ARMOR)) {
                        if (SEHelper.getSoulsAmount(player, SoulUse(player, stack))) {
                            ...
                            if (hostile != null || player.hurtTime > 0) {
                                ...
                                int duration = 1;
                                if (stack.isEnchanted()) {
                                    enchantment = EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.POTENCY, stack);
                                    duration += EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.DURATION, stack);
                                }
                                ...
                                worldIn.playSound(null, player.getX(), player.getY(), player.getZ(), ModSounds.IRON_HIDE, SoundSource.PLAYERS, 1.0F, 1.0F);
                            }
                        }
                    }
                }
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack, Enchantment enchantment) {
        return enchantment == ModEnchantments.POTENCY
                || enchantment == ModEnchantments.DURATION;
    }
