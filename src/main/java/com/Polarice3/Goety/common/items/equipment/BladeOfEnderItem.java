...
public class BladeOfEnderItem extends SwordItem implements IPersist, ISoulRepair, IEnchantment {

    public BladeOfEnderItem() {
        super(ModTiers.VOID, Mth.floor(ItemConfig.BladeOfEnderDamage.get() - 1), -(4.0F - ItemConfig.BladeOfEnderAttackSpeed.get().floatValue()),
                new FabricItemSettings().customDamage(BladeOfEnderItem::damageItem).durability(ItemConfig.BladeOfEnderDurability.get()).fireResistant());
    }
    ...
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(ItemStack stack, EquipmentSlot slot) {
        if (this.isNotBroken(stack) || !ItemConfig.BladeOfEnderPersist.get()) {
            Multimap<Attribute, AttributeModifier> map = MultimapBuilder.hashKeys().hashSetValues().build(super.getAttributeModifiers(stack, slot));
            map.put(ReachEntityAttributes.ATTACK_RANGE, new AttributeModifier(ModUUIDUtil.createUUID("item.goety.boe.reach"), "Tool Modifier", 1, AttributeModifier.Operation.ADDITION));
            return slot == EquipmentSlot.MAINHAND ? map : super.getAttributeModifiers(stack, slot);
        } else {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!(ItemConfig.BladeOfEnderPersist.get() && this.isBroken(itemstack))) {
            if (!playerIn.getCooldowns().isOnCooldown(this)
                    && !offhandItem./*canPerformAction(ToolActions.SHIELD_BLOCK)*/is(ConventionalItemTags.SHIELDS) && !(offhandItem.getItem() instanceof ShieldItem)
                ...
                if (playerIn.isSprinting()) {
                    ...
                    if (worldIn instanceof ServerLevel serverLevel) {
                        ModNetwork.sentToTrackingEntityAndPlayer(playerIn, SPlayFollowSoundPacket.ID, SPlayFollowSoundPacket.encode(playerIn, ModSounds.VHOE_CHARGE, 3.0F, playerIn.getVoicePitch(), false));
                    ...
                    if (!worldIn.isClientSide) {
                        ModNetwork.sentToTrackingEntityAndPlayer(playerIn, SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(playerIn.blockPosition(), SoundEvents.PLAYER_ATTACK_CRIT, 2.0F, 1.0F));
                    } else {
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack, Enchantment enchantment) {
        return enchantment.category.canEnchant(stack.getItem()) || enchantment == ModEnchantments.VELOCITY || enchantment == ModEnchantments.RADIUS;
    }
    ...
    public boolean isValidRepairItem(ItemStack pToRepair, ItemStack pRepair) {
        return pRepair.is(ConventionalItemTags.NETHERITE_INGOTS);
    }
    ...
    public static void emptyClick(ItemStack stack) {
        if (!stack.isEmpty() && stack.getItem() instanceof BladeOfEnderItem blade) {
            if (blade.isNotBroken(stack) || !ItemConfig.BladeOfEnderPersist.get()) {
                ModNetwork.sendToServer(CBoEStrikePacket.ID, CBoEStrikePacket.DUMMY);
            }
        }
    }
    ...
    public static void strike(Level pLevel, Player pPlayer) {
        if (pPlayer.getAttackStrengthScale(0.5F) > 0.9F) {
            pLevel.playSound(null, pPlayer.getX(), pPlayer.getY(), pPlayer.getZ(), ModSounds.OBSIDIAN_CLAYMORE_SWING, SoundSource.PLAYERS, 2.0F, pPlayer.getVoicePitch());
            if (!pLevel.isClientSide) {
                ItemStack sword = pPlayer.getMainHandItem();
                float speed = 0.0F;
                float radius = 0.0F;
                if (sword.isEnchanted()) {
                    speed += EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.VELOCITY, sword) / 3.0F;
                    radius += EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.RADIUS, sword) / 4.0F;
                }
