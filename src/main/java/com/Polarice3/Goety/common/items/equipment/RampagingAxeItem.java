        ...
        if (pState.is(BlockTags.MINEABLE_WITH_AXE)) {
            ...
            if (!pEntityLiving.hasEffect(GoetyEffects.RAMPAGE)) {
                ...
                if (effectinstance1.getAmplifier() < 4 && pLevel.random.nextFloat() <= 0.25F) {
                    EffectsUtil.amplifyEffect(pEntityLiving, GoetyEffects.RAMPAGE, MathHelper.secondsToTicks(10));
                } else {
                    EffectsUtil.resetDuration(pEntityLiving, GoetyEffects.RAMPAGE, MathHelper.secondsToTicks(10));
                }
            }
        }
