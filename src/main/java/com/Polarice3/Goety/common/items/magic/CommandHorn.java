    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isCrouching()) {
            ...
            if (level instanceof ServerLevel serverlevel) {
                                ...
                                if (isNone(itemstack)) {
                                    if (!servant.isGuardingArea()
                                        ...
                                        if (servant.isStaying() || servant.isWandering()) {
                                            ...
                                            serverlevel.sendParticles(ModParticleTypes.GO, livingEntity.getX(), livingEntity.getY(), livingEntity.getZ(), 0, 0, 2.0D, 0, 1.0F);
                                        } else {
                                            servant.setBoundPos(null);
                                            servant.setWandering(false);
                                            servant.setStaying(true);
                                            serverlevel.sendParticles(ModParticleTypes.STOP, livingEntity.getX(), livingEntity.getY(), livingEntity.getZ(), 0, 0, 2.0D, 0, 1.0F);
                                        }
                                    }
                ...
                player.playSound(ModSounds.WIND_HORN, 1.0F, 0.45F);
                ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(ModSounds.WIND_HORN, 1.0F, 0.45F));
                level.gameEvent(player, GameEvent.INSTRUMENT_PLAY, player.position());
            }
        }
