...
public class DarkWand extends Item implements IWand, ILeftClickEntity, ICustomArmPose, IForgeHandTransform {
    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (entityIn instanceof LivingEntity livingEntity) {
            ...
            var handler = SoulUsingItemHandler.get(stack);
            if (handler.getSlot().getItem() instanceof IFocus) {
                handler.getSlot().inventoryTick(worldIn, entityIn, itemSlot, isSelected);
        ...
        if (entity instanceof LivingEntity target && target instanceof IOwned owned && (owned.getTrueOwner() == player || (owned.getTrueOwner() instanceof IOwned owned1 && owned1.getTrueOwner() == player))) {
            if (!player.level.isClientSide) {
                ...
                if (focus.getItem() instanceof CallFocus && !CallFocus.hasSummon(focus)) {
                    ...
                    if (focus.hasTag()) {
                        compoundTag = focus.getTag();
                    }
                    CallFocus.setSummon(compoundTag, target);
                    focus.setTag(compoundTag);
                    player.playSound(SoundEvents.ARROW_HIT_PLAYER, 1.0F, 0.45F);
                    ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.ARROW_HIT_PLAYER, 1.0F, 0.45F));
                    flag = true;
                    ...
                    ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.ARROW_HIT_PLAYER, 1.0F, 0.45F));
                    flag = true;
                } else if (focus.getItem() instanceof CommandFocus && owned instanceof IServant servant && servant.canBeCommanded() && !CommandFocus.hasServant(focus)) {
                    CompoundTag compoundTag = new CompoundTag();
                    if (focus.hasTag()) {
                        compoundTag = focus.getTag();
                    }
                    ...
                    ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.ARROW_HIT_PLAYER, 1.0F, 0.45F));
                    flag = true;
                } else if (focus.getItem() instanceof OrderFocus && owned instanceof IServant servant && servant.canBeCommanded()) {
                    List<LivingEntity> list = OrderFocus.getServants(focus);
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (focus.getItem() instanceof CommandFocus) {
            if (CommandFocus.getServant(focus) instanceof IServant summoned && summoned != target) {
                if (summoned.getTrueOwner() == player && target.distanceTo(player) <= 64) {
                    summoned.setCommandPosEntity(target);
                    player.playSound(ModSounds.COMMAND, 1.0F, 0.45F);
                    if (!player.level.isClientSide) {
                        ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(ModSounds.COMMAND, 1.0F, 0.45F));
                    }
                    return InteractionResult.SUCCESS;
                }
            }
        } else if (focus.getItem() instanceof OrderFocus) {
            if (!OrderFocus.getServants(focus).isEmpty()) {
                int i = 0;
                for (LivingEntity livingEntity : OrderFocus.getServants(focus)) {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if (player != null) {
            var handler = SoulUsingItemHandler.get(stack);
            ItemStack focus = handler.getSlot();
            if (focus.getItem() instanceof RecallFocus recallFocus) {
                CompoundTag compoundTag = focus.getOrCreateTag();
            ...
            } else if (focus.getItem() instanceof CommandFocus) {
                if (CommandFocus.hasServant(focus) && CommandFocus.getServant(focus) instanceof IServant summoned) {
                    LivingEntity livingEntity = CommandFocus.getServant(focus);
                ...
                if (!level.isClientSide) {
                    ...
                    if (compoundtag != null) {
                        if (compoundtag.contains("Patterns")) {
                            ...
                            level.playSound(null, blockpos.getX(), blockpos.getY(), blockpos.getZ(), ModSounds.CAST_SPELL, SoundSource.BLOCKS, 1.0F, 0.5F);
                            return InteractionResult.SUCCESS;
                        }
                    }
                }
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity livingEntity, int useTimeRemaining) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            if (spell != null) {
                spell.stopSpell(serverLevel, livingEntity, stack, focus, CastTime, WandUtil.getStats(livingEntity, spell));
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        var handler = SoulUsingItemHandler.get(itemstack);
        ItemStack focus = handler.getSlot();
        if (focus.getItem() instanceof CommandFocus && playerIn.isCrouching()) {
            if (CommandFocus.hasServant(focus) && focus.getTag() != null) {
                ...
                if (!worldIn.isClientSide) {
                    ModNetwork.sendTo(playerIn, SPlayEntitySoundPacket.ID, SPlayEntitySoundPacket.encode(playerIn.getUUID(), SoundEvents.ARROW_HIT_PLAYER, 1.0F, 0.45F));
                }
            }
    ...
    public boolean applyForgeHandTransform(PoseStack poseStack, LocalPlayer player, HumanoidArm arm, ItemStack itemInHand, float partialTick, float equipProcess, float swingProcess) {
        ...
        if (player.isUsingItem()) {
            ...
            poseStack.translate((float) i * -0.2785682F, 0.18344387F, (double) 0.15731531F);
