    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown() || player.isCrouching()) {
            ...
            if (!level.isClientSide) {
                ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.BOOK_PAGE_TURN, 1.0F, 0.5F));
            }
        ...
        if (!level.isClientSide()) {
            ...
            if (togglePos(stack, entry)) {
                if (player != null) {
                    ...
                    if (!level.isClientSide) {
                        ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.BOOK_PAGE_TURN, 1.0F, 1.0F));
                    }
                }
            } else {
                if (player != null) {
