                ...
                if (blockPos != null) {
                    if (getDimension(stack.getTag()).get() == livingEntity.level.dimension()) {
                        ...
                        if (optional.isPresent()) {
                            var event = EventHooks.onEnderTeleport(livingEntity, optional.get().x, optional.get().y, optional.get().z);
                            if (event.isCanceled()) {
                                return false;
                            }
                            livingEntity.teleportTo(event.getTargetX(), event.getTargetY(), event.getTargetZ());
                            if (livingEntity.getServer() != null) {
                                ModNetwork.sendToALL(livingEntity.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(BlockPos.containing(livingEntity.xo, livingEntity.yo, livingEntity.zo), SoundEvents.ENDERMAN_TELEPORT, 1.0F, 1.0F));
                                ModNetwork.sendToALL(livingEntity.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(BlockPos.containing(optional.get()), SoundEvents.ENDERMAN_TELEPORT, 1.0F, 1.0F));
                            }
                            return true;
                        }
                    } else {
                        if (livingEntity.getServer() != null) {
                            ...
                            if (serverWorld != null) {
                                ...
                                if (optional.isPresent()) {
                                    ...
                                    FabricDimensions.teleport(livingEntity, serverWorld, new ArcaTeleporter(optional.get(), livingEntity));
                                    livingEntity.teleportTo(event.getTargetX(), event.getTargetY(), event.getTargetZ());
                                    return true;
                                }
                            }
