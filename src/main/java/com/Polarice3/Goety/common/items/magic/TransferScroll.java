...
public class TransferScroll extends ItemBase implements ILeftClickEntity {
                    ...
                    if (summon instanceof IOwned owned) {
                        if (!level.isClientSide) {
                            if (owned.getTrueOwner() == player) {
                                ...
                                ModNetwork.sendTo(player, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(SoundEvents.RESPAWN_ANCHOR_SET_SPAWN, 1.0F, 1.0F));
                                itemstack.shrink(1);
                                return InteractionResultHolder.success(itemstack);
                            }
                        }
