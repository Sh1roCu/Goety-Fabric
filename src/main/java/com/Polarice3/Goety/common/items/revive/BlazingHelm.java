...
public class BlazingHelm extends ReviveServantItem {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null) {
            ...
            entity = new Wildfire(ModEntityType.WILDFIRE, level);
        ...
        if (entity instanceof Wildfire wildfire) {
            ...
            if (flag) {
                if (wildfire.getTrueOwner() == player) {
                    if (RitualRequirements.canSummon(level, player, ModEntityType.WILDFIRE)) {
                        ...
                        if (level.addFreshEntity(wildfire)) {
                            ...
                            if (level instanceof ServerLevel serverLevel) {
                                for (int i = 0; i < 8; ++i) {
                                    ServerParticleUtil.addParticlesAroundSelf(serverLevel, ModParticleTypes.BIG_FIRE, wildfire);
                            ...
                            wildfire.playSound(ModSounds.WILDFIRE_AMBIENT, 2.0F, 0.5F);
                            target.discard();
                            player.swing(hand);
