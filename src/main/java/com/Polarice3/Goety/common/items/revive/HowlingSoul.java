...
public class HowlingSoul extends ReviveServantItem {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null) {
            ...
            entity = new BlackBeast(ModEntityType.BLACK_BEAST, level);
        ...
        if (entity instanceof BlackBeast blackBeast) {
            ...
            if (flag) {
                if (blackBeast.getTrueOwner() == player) {
                    if (RitualRequirements.canSummon(level, player, ModEntityType.BLACK_BEAST)) {
                        ...
                        if (level.addFreshEntity(blackBeast)) {
                            ...
                            blackBeast.playSound(ModSounds.BLACK_BEAST_ROAR, 2.0F, 0.5F);
                            target.discard();
                            player.swing(hand);
