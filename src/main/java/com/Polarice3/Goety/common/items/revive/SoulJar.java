...
public class SoulJar extends ReviveServantItem {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null) {
            ...
            entity = new NecromancerServant(ModEntityType.NECROMANCER_SERVANT, level);
            if (isCairn(stack)) {
                entity = new CairnNecromancerServant(ModEntityType.CAIRN_NECROMANCER_SERVANT, level);
            } else if (isMossy(stack)) {
                entity = new MossyNecromancerServant(ModEntityType.MOSSY_NECROMANCER_SERVANT, level);
            } else if (isDrowned(stack)) {
                entity = new DrownedNecromancer(ModEntityType.DROWNED_NECROMANCER_SERVANT, level);
            } else if (isWither(stack)) {
                entity = new WitherNecromancerServant(ModEntityType.WITHER_NECROMANCER_SERVANT, level);
            }
        ...
        if (entity instanceof AbstractNecromancer necromancer) {
            ...
            if (flag) {
                if (necromancer.getTrueOwner() == player) {
                    if (RitualRequirements.canSummon(level, player, ModEntityType.NECROMANCER_SERVANT)) {
                        ...
                        if (level.addFreshEntity(necromancer)) {
                            ...
                            if (necromancer instanceof DrownedNecromancer) {
                                necromancer.playSound(ModSounds.DROWNED_NECROMANCER_AMBIENT, 2.0F, 0.5F);
                            } else {
                                necromancer.playSound(ModSounds.NECROMANCER_LAUGH, 2.0F, 0.5F);
                            }
                            target.discard();
                            player.swing(hand);
    ...
    public static AbstractNecromancer getNecromancer(ItemStack stack, Level level) {
        ...
        if (itemTag != null) {
            ...
            ResourceLocation location = new ResourceLocation(entityTag.getString("entity"));
