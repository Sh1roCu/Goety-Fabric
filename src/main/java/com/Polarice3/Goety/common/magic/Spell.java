...
public abstract class Spell implements ISpell {
    ...
    public boolean GeoPower(LivingEntity caster) {
        return CuriosFinder.hasCurio(caster, ModItems.AMETHYST_NECKLACE);
    }
    ...
    public void useParticle(Level worldIn, LivingEntity caster, ItemStack stack) {
        if (worldIn instanceof ServerLevel serverLevel) {
            if (this.getSpellType() == SpellType.FROST) {
                ...
                if (caster.tickCount % 5 == 0) {
                    ...
                    if (caster instanceof Player player) {
                        for (int i = 0; i < (caster.getRandom().nextFloat() < 0.1F ? 3 : 1); i++) {
                            ModNetwork.sentToTrackingEntityAndPlayer(player, SStaffParticlePacket.ID, SStaffParticlePacket.encode(player.getId(), stack.getItem() instanceof IWand wand ? wand.getWandVisualHeight(serverLevel, player, stack) : 0.8F, range, color, caster.getUsedItemHand() == InteractionHand.OFF_HAND));
                        }
                    } else {
                ...
                if (caster.tickCount % 5 == 0) {
                    ServerParticleUtil.addParticlesAroundMiddleSelf(serverLevel, ModParticleTypes.SPELL_ELECTRIC, caster);
                }
            } else if (this.getSpellType() == SpellType.WILD) {
                ColorUtil colorUtil = new ColorUtil(0xfcd9f7);
                serverLevel.sendParticles(ModParticleTypes.SPELL_SQUARE, caster.getX(), caster.getY() + 2.0D, caster.getZ(), 0, colorUtil.red(), colorUtil.green(), colorUtil.blue(), 0.5F);
                ...
                if (caster instanceof Player player) {
                    for (int i = 0; i < (caster.getRandom().nextFloat() < 0.1F ? 3 : 1); i++) {
                        ModNetwork.sentToTrackingEntityAndPlayer(player, SStaffParticlePacket.ID, SStaffParticlePacket.encode(player.getId(), stack.getItem() instanceof IWand wand ? wand.getWandVisualHeight(serverLevel, player, stack) : 0.8F, range, colorUtil.colorCode(0), caster.getUsedItemHand() == InteractionHand.OFF_HAND));
                    }
                } else {
