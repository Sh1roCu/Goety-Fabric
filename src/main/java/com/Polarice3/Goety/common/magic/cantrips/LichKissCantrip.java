...
public class LichKissCantrip {
        ...
        if (pPlayer != null && !pPlayer.isSpectator()) {
            ...
            if (target instanceof LivingEntity livingEntity) {
                ...
                if (livingEntity.hurt(ModDamageSource.soulLeech(pPlayer, pPlayer), 1.0F)) {
                    ...
                    pPlayer.playSound(ModSounds.SOUL_EAT, 1.0F, 1.0F);
                    ModNetwork.sendTo(pPlayer, SPlayPlayerSoundPacket.ID, SPlayPlayerSoundPacket.encode(ModSounds.SOUL_EAT, 1.0F, 1.0F));
                }
            }
        }
    }
    ...
    private void drawParticleBeam(LivingEntity pSource, LivingEntity pTarget) {
        ...
        if (!pSource.level.isClientSide) {
            ...
            while (d4 < d3) {
                d4 += 1.0D;
                serverWorld.sendParticles(ModParticleTypes.LEECH, pSource.getX() + d0 * d4, pSource.getY() + d1 * d4 + (double) pSource.getEyeHeight() * 0.5D, pSource.getZ() + d2 * d4, 1, 0.0D, 0.0D, 0.0D, 0.0D);
            }
        }
    }
}
