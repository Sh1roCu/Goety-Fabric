...
public class FireBreathSpell extends BreathingSpell {
    ...
    public SoundEvent CastingSound() {
        return ModSounds.FIRE_BREATH_START;
    }

    @Override
    public SoundEvent loopSound(LivingEntity caster) {
        return ModSounds.FIRE_BREATH;
    }
    ...
    public void showWandBreath(LivingEntity entityLiving, ItemStack staff, SpellStat spellStat) {
        ...
        if (!CuriosFinder.hasCurio(entityLiving, ModItems.RING_OF_THE_DRAGON)) {
            this.dragonBreathAttack(ModParticleTypes.SMALL_DRAGON_FLAME, entityLiving, 10, ((double) range / 10) * 0.5D, 1.0D);
        } else {
            this.dragonBreathAttack(ModParticleTypes.DRAGON_FLAME, entityLiving, ((double) range / 10) * 0.5D);
        }
    }
}
