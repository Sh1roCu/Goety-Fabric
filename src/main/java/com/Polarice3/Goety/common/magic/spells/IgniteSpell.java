...
//        double d0 = caster.getAttributeValue(ForgeMod.BLOCK_REACH);
//        double entityReach = caster.getAttributeValue(ForgeMod.ENTITY_REACH);
        double d0 = ReachEntityAttributes.getReachDistance(caster, 4.5);
        double entityReach = ReachEntityAttributes.getAttackRange(caster, 3);
    ...
    public void touchResult(ServerLevel worldIn, LivingEntity caster, LivingEntity target, ItemStack staff, SpellStat spellStat) {
        ...
        target.setSecondsOnFire(SpellConfig.IgniteFireSeconds.get() + spellStat.getBurning() + WandUtil.getLevels(ModEnchantments.BURNING, caster));
    }
}
