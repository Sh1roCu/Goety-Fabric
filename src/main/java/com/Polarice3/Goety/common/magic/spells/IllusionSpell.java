...
public class IllusionSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!this.isShifting(caster)) {
            ...
            boolean undead = CuriosFinder.hasNamelessSet(caster) && staff.is(ModItems.NAMELESS_STAFF);
            ParticleOptions particleOptions = ParticleTypes.POOF;
            if (undead) {
                particleOptions = ModParticleTypes.LICH;
            }
            for (int i1 = 0; i1 < i0; ++i1) {
                Doppelganger summonedentity = new Doppelganger(ModEntityType.DOPPELGANGER, worldIn);
                summonedentity.setTrueOwner(caster);
                if (caster instanceof Player player) {
                    ModNetwork.sendTo(player, SSetPlayerOwnerPacket.ID, SSetPlayerOwnerPacket.encode(summonedentity));
                }
                ...
                if (undead) {
                    ...
                    if (LichdomHelper.lichModeColor(caster) > -1) {
                        ...
                        for (int i = 0; i < worldIn.getRandom().nextInt(10) + 10; ++i) {
                            worldIn.sendParticles(ModParticleTypes.LICH_COLORED, caster.getX(), caster.getEyeY(), caster.getZ(), 0, colorUtil.red(), colorUtil.green(), colorUtil.blue(), 1.0F);
                        }
                        flag = false;
                    }
            ...
            boolean flag = true;
            if (undead) {
                soundEvent = ModSounds.LICH_TELEPORT_IN;
                if (LichdomHelper.lichModeColor(caster) > -1) {
