...
public class ShockwaveSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        for (int i = -radius; i < radius; ++i) {
            for (int k = -radius; k < radius; ++k) {
                ...
                if (worldIn.getRandom().nextFloat() <= 0.05F) {
                    worldIn.sendParticles(ModParticleTypes.SOUL_EXPLODE, blockPos.getX(), blockPos.getY(), blockPos.getZ(), 0, 0, 0.04D, 0, 0.5F);
                }
            }
        }
        ...
        ModNetwork.sendToALL(worldIn.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(caster.blockPosition(), ModSounds.SOUL_EXPLODE, 2.0F, 1.0F));
