            ...
            if (this.isShifting(caster)) {
                ...
                for (int length = 0; length < 16; length++) {
                    ...
                    if (worldIn.noCollision(new AABB(blockpos$mutable))) {
                        ViciousTooth viciousTooth = new ViciousTooth(ModEntityType.VICIOUS_TOOTH, worldIn);
    ...
    public void surroundTeeth(LivingEntity livingEntity, BlockPos blockPos, float damage, boolean isStaff) {
        ...
        if (isStaff) {
            for (int i = 0; i < 5; ++i) {
                ...
                ViciousTooth viciousTooth = new ViciousTooth(ModEntityType.VICIOUS_TOOTH, livingEntity.level);
