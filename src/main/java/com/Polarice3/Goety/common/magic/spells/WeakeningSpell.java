    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        for (int i = -radius; i < radius; ++i) {
            for (int k = -radius; k < radius; ++k) {
                ...
                if (worldIn.random.nextFloat() <= 0.25F) {
                    worldIn.sendParticles(ModParticleTypes.SPELL_SQUARE, blockPos.getX(), blockPos.getY(), blockPos.getZ(), 0, colorUtil.red(), colorUtil.green(), colorUtil.blue(), 0.5F);
                }
            }
        }
        for (LivingEntity livingEntity : worldIn.getEntitiesOfClass(LivingEntity.class, caster.getBoundingBox().inflate(radius))) {
            if (!MobUtil.areAllies(caster, livingEntity)) {
                ...
                livingEntity.playSound(ModSounds.WEAKEN_CURSE);
            }
        }
