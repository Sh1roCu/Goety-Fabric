...
public class FrostBreathSpell extends BreathingSpell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!worldIn.isClientSide) {
            if (CuriosFinder.hasCurio(caster, ModItems.RING_OF_THE_DRAGON)) {
                damage *= 2.0F;
                if (SpellConfig.DragonFrostGriefing.get()) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        if (hitResult.getType() == HitResult.Type.BLOCK) {
                            ...
                            if (worldIn.getBlockState(blockPos) == FrostedIceBlock.meltsInto() && blockstate.canSurvive(worldIn, blockPos) && worldIn.isUnobstructed(blockstate, blockPos, CollisionContext.empty()) /*&& !EventHooks.onBlockPlace(caster, net.minecraftforge.common.util.BlockSnapshot.create(worldIn.dimension(), worldIn, blockPos), net.minecraft.core.Direction.UP)*/) {
        ...
        worldIn.playSound(null, caster.getX(), caster.getY(), caster.getZ(), ModSounds.FROST_BREATH, this.getSoundSource(), worldIn.random.nextFloat() * 0.5F, worldIn.random.nextFloat() * 0.5F);
    }
    ...
    public void showWandBreath(LivingEntity entityLiving, ItemStack staff, SpellStat spellStat) {
        ...
        if (!CuriosFinder.hasCurio(entityLiving, ModItems.RING_OF_THE_DRAGON)) {
