...
public class FrostNovaSpell extends Spell {
    ...
    public SoundEvent CastingSound() {
        return ModSounds.FROST_PREPARE_SPELL;
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        int finalAmp = amp;
        new SpellExplosion(worldIn, caster, ModDamageSource.directFreeze(caster), spellTarget.blockPosition(), radius, trueDamage) {
            @Override
        ...
        this.playSound(worldIn, spellTarget, ModSounds.ICE_CHUNK_HIT, 1.0F, 0.5F);
        ...
        for (int i = -radius; i <= radius; ++i) {
            for (int j = -radius; j <= radius; ++j) {
                for (int k = -radius; k <= radius; ++k) {
                    ...
                    serverLevel.sendParticles(ModParticleTypes.FROST_NOVA, d0, d1, d2, 0, d3 / d6, d4 / d6, d5 / d6, 0.5F);
