...
public class BarricadeSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        EntityType<? extends AbstractMonolith> entityType = ModEntityType.TOTEMIC_WALL;
        if (this.typeStaff(staff, SpellType.FROST)) {
            entityType = ModEntityType.GLACIAL_WALL;
        }
        if (target != null) {
            if (this.isShifting(caster)) {
                if (worldIn.getRandom().nextFloat() <= chance) {
                    WandUtil.summonQuadOffensiveTrap(caster, target, ModEntityType.TOTEMIC_BOMB, potency);
                    cooldown += MathHelper.secondsToTicks(3);
                } else {
                    int xShift = worldIn.getRandom().nextInt(-1, 1);
                    int zShift = worldIn.getRandom().nextInt(-1, 1);
                    WandUtil.summonMonolith(caster, target, ModEntityType.TOTEMIC_BOMB, xShift, zShift, potency);
            ...
            if (this.isShifting(caster)) {
                if (worldIn.random.nextFloat() <= chance) {
                    WandUtil.summonQuadOffensiveTrap(caster, blockPos, ModEntityType.TOTEMIC_BOMB, potency);
                    cooldown += MathHelper.secondsToTicks(3);
                } else {
                    WandUtil.summonMonolith(caster, blockPos, ModEntityType.TOTEMIC_BOMB, 0, 0, potency);
                    cooldown += MathHelper.secondsToTicks(2);
                }
            } else {
