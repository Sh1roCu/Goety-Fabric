...
public class BlackguardSpell extends SummonSpell {
    ...
    public SoundEvent loopSound(LivingEntity caster) {
        return ModSounds.VANGUARD_SPELL;
    }
    ...
    public void commonResultHit(ServerLevel worldIn, LivingEntity caster) {
        for (int i = 0; i < caster.level.random.nextInt(35) + 10; ++i) {
            worldIn.sendParticles(ModParticleTypes.LICH, caster.getX(), caster.getEyeY(), caster.getZ(), 1, 0.0F, 0.0F, 0.0F, 0);
        }
        this.playSound(worldIn, caster, ModSounds.VANGUARD_SUMMON);
    }
        ...
        if (!isShifting(caster)) {
            if (staff.is(ModItems.NAMELESS_STAFF)) {
                ...
                for (int i1 = -3; i1 <= 3; ++i1) {
                    BlackguardServant summonedentity = new BlackguardServant(ModEntityType.BLACKGUARD_SERVANT, worldIn);
                    ...
                    if (worldIn.addFreshEntity(summonedentity)) {
                        worldIn.sendParticles(ModParticleTypes.LICH, summonedentity.getX(), summonedentity.getY(), summonedentity.getZ(), 1, 0, 0, 0, 0.0F);
                        ServerParticleUtil.summonPowerfulUndeadParticles(worldIn, summonedentity);
                        this.playSound(worldIn, summonedentity, ModSounds.SOUL_EXPLODE, 0.25F + (worldIn.random.nextFloat() / 2.0F), 1.0F);
