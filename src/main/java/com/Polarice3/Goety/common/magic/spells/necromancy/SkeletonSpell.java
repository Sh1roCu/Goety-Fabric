...
public class SkeletonSpell extends SummonSpell {
    ...
    public boolean specialStaffs(ItemStack stack) {
        return typeStaff(stack, SpellType.FROST)
                ...
                || stack.is(ModItems.OMINOUS_STAFF);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            for (int i1 = 0; i1 < i; ++i1) {
                ...
                AbstractSkeletonServant summonedentity = new SkeletonServant(ModEntityType.SKELETON_SERVANT, worldIn);
                ...
                if (specialStaffs(staff)) {
                    if (typeStaff(staff, SpellType.FROST)) {
                        summonedentity = new StrayServant(ModEntityType.STRAY_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.STORM)) {
                        summonedentity = new RattledServant(ModEntityType.RATTLED_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.WILD)) {
                        summonedentity = new MossySkeletonServant(ModEntityType.MOSSY_SKELETON_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.NETHER)) {
                        summonedentity = new WitherSkeletonServant(ModEntityType.WITHER_SKELETON_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.ABYSS)) {
                        summonedentity = new SunkenSkeletonServant(ModEntityType.SUNKEN_SKELETON_SERVANT, worldIn);
                    } else if (staff.is(ModItems.OMINOUS_STAFF)) {
                        summonedentity = new SkeletonPillagerServant(ModEntityType.SKELETON_PILLAGER_SERVANT, worldIn);
                    }
                } else {
                    Player player = null;
                ...
                if (summonedentity.getType() != ModEntityType.SUNKEN_SKELETON_SERVANT) {
