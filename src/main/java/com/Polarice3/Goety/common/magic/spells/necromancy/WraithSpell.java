    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                AbstractWraith summonedentity = new AbstractWraith(ModEntityType.WRAITH_SERVANT, worldIn);
                Player player = null;
                if (caster instanceof Player player1) {
                    player = player1;
                }
                if (typeStaff(staff, SpellType.WILD)) {
                    summonedentity = new AbstractMuckWraith(ModEntityType.MUCK_WRAITH_SERVANT, worldIn);
                } else {
