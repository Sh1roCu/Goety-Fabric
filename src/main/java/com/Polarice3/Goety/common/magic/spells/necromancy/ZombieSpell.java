    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                ...
                Summoned summonedentity = new ZombieServant(ModEntityType.ZOMBIE_SERVANT, worldIn);
                ...
                if (specialStaffs(staff)) {
                    if (typeStaff(staff, SpellType.FROST)) {
                        summonedentity = new FrozenZombieServant(ModEntityType.FROZEN_ZOMBIE_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.STORM)) {
                        summonedentity = new FrayedServant(ModEntityType.FRAYED_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.WILD)) {
                        summonedentity = new JungleZombieServant(ModEntityType.JUNGLE_ZOMBIE_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.NETHER)) {
                        summonedentity = new ZPiglinServant(ModEntityType.ZPIGLIN_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.ABYSS)) {
                        summonedentity = new DrownedServant(ModEntityType.DROWNED_SERVANT, worldIn);
                    } else if (staff.is(ModItems.OMINOUS_STAFF)) {
                        summonedentity = new ZombieVindicatorServant(ModEntityType.ZOMBIE_VINDICATOR_SERVANT, worldIn);
                    }
                } else {
                    Player player = null;
                ...
                if (summonedentity.getType() != ModEntityType.DROWNED_SERVANT) {
