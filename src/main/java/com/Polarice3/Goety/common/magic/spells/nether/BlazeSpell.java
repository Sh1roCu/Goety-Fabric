    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                BlazeServant blazeServant = new BlazeServant(ModEntityType.BLAZE_SERVANT, worldIn);
                if (CuriosFinder.hasUnholySet(caster)) {
                    blazeServant = new Inferno(ModEntityType.INFERNO, worldIn);
                }
            ...
            this.playSound(worldIn, caster, ModSounds.SUMMON_SPELL_FIERY);
        }
    }
