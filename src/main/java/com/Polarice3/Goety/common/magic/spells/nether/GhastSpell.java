    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                ...
                Malghast ghast = new MiniGhast(ModEntityType.MINI_GHAST, worldIn);
                if (this.NetherPower(caster)) {
                    ghast = new GhastServant(ModEntityType.GHAST_SERVANT, worldIn);
                    if (CuriosFinder.hasUnholySet(caster)) {
                        ghast = new Malghast(ModEntityType.MALGHAST, worldIn);
                    }
            ...
            worldIn.playSound(null, caster.getX(), caster.getY(), caster.getZ(), ModSounds.SUMMON_SPELL, this.getSoundSource(), 1.0F, 1.0F);
        }
    }
}
