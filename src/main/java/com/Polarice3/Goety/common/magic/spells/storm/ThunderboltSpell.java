...
public class ThunderboltSpell extends Spell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (lightningRod.isPresent() && !rightStaff(staff)) {
            ...
            ModNetwork.sendToALL(worldIn.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, new Vec3(blockPos.getX(), blockPos.getY(), blockPos.getZ()), colorUtil, 10));
            this.playSound(worldIn, caster, ModSounds.THUNDERBOLT);
        } else {
            ...
            if (livingEntity != null && ServerLivingEntityEvents.ALLOW_DAMAGE.invoker().allowDamage(livingEntity, ModDamageSource.directShock(caster), damage)) {
                ...
                ModNetwork.sendToALL(worldIn.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, vec31, colorUtil, 10));
