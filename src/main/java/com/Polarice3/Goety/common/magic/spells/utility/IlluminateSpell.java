...
public class IlluminateSpell extends Spell {
        ...
        if (castTime >= SpellConfig.IlluminateChargeUp.get()) {
            ...
            if (castTime % SpellConfig.IlluminateDuration.get() == 0) {
                if (stream.anyMatch(pos -> tryToPlaceAtPos(worldIn, caster, pos) || replaceSoulLight(pos, worldIn))) {
                    ServerParticleUtil.addParticlesAroundMiddleSelf(worldIn, ModParticleTypes.GLOW_EFFECT, caster);
                }
            }
        }
    }
    ...
    public boolean replaceSoulLight(BlockPos pos, ServerLevel world) {
        if (world.getBlockState(pos).is(ModBlocks.SOUL_LIGHT_BLOCK)) {
            if (world.setBlock(pos, ModBlocks.GLOW_LIGHT_BLOCK.defaultBlockState(), 3)) {
                world.sendParticles(ModParticleTypes.GLOW_EFFECT, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 1, 0.0F, 0.0F, 0.0F, 0.0F);
    ...
    public boolean tryToPlaceGlowLight(BlockPos pos, ServerLevel world) {
        if (ModBlocks.GLOW_LIGHT_BLOCK.defaultBlockState().canSurvive(world, pos)) {
