...
public class BanishSpell extends TouchSpell {
        ...
        if (hitResult instanceof EntityHitResult result){
            if (result.getEntity() instanceof LivingEntity living){
                if (living.getMaxHealth() > SpellConfig.BanishMaxHealth.get() || living.getType().is(ModTags.EntityTypes.BANISH_IMMUNE) || MobUtil.hasEntityTypesConfig(SpellConfig.BanishBlackList.get(), living.getType())){
                    this.playSound(worldIn, caster, ModSounds.SPELL_FAIL, 1.0F, 1.0F);
                    return false;
                }
                var event = EventHooks.onEnderTeleport(living, caster.getX(), caster.getY(), caster.getZ());
                if (event.isCanceled()) {
                    return false;
                }
            }
    ...
    public void touchResult(ServerLevel worldIn, LivingEntity caster, LivingEntity target, ItemStack staff, SpellStat spellStat) {
        ...
        for (int i = 0; i < range; ++i) {
            ...
            double d4 = Mth.clamp(target.getY() + (double) (RandomUtil.nextInt(target.getRandom(), range) - (range / 2)), worldIn.getMinBuildHeight(), worldIn.getMinBuildHeight() + worldIn.getLogicalHeight() - 1);
