...
public class HuntingSpell extends SummonSpell {
    ...
    public SoundEvent CastingSound() {
        return ModSounds.WILD_PREPARE_SPELL;
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                Summoned summonedentity = new BlackWolf(ModEntityType.BLACK_WOLF, worldIn);
                ...
                if (this.typeStaff(staff, SpellType.NECROMANCY)) {
                    summonedentity = new SkeletonWolf(ModEntityType.SKELETON_WOLF, worldIn);
                } else if ((worldIn.isThundering() && worldIn.canSeeSky(blockPos)) || this.typeStaff(staff, SpellType.STORM)) {
                    summonedentity = new Stormhound(ModEntityType.STORMHOUND, worldIn);
                } else if (worldIn.getBiome(blockPos).is(ConventionalBiomeTags.MOUNTAIN) || this.typeStaff(staff, SpellType.WIND)) {
                    summonedentity = new TwilightGoat(ModEntityType.TWILIGHT_GOAT, worldIn);
                } else if (worldIn.isWaterAt(blockPos) || this.typeStaff(staff, SpellType.ABYSS)) {
                    summonedentity = new Snapper(ModEntityType.SNAPPER, worldIn);
                } else if (worldIn.getBiome(blockPos).value().coldEnoughToSnow(blockPos) || this.typeStaff(staff, SpellType.FROST)) {
                    summonedentity = new WinterWolf(ModEntityType.WINTER_WOLF, worldIn);
                    ...
                    summonedentity = new Hellhound(ModEntityType.HELLHOUND, worldIn);
                }
                summonedentity.setTrueOwner(caster);
                summonedentity.moveTo(blockPos, 0.0F, 0.0F);
                if (summonedentity.getType() != ModEntityType.SNAPPER) {
            ...
            this.playSound(worldIn, caster, ModSounds.SUMMON_SPELL);
        }
    }
    ...
    public void summonParticles(ServerLevel worldIn, LivingEntity caster, ItemStack staff, LivingEntity summoned) {
        ...
        if (summoned.getType() == ModEntityType.BLACK_WOLF) {
            colorUtil = new ColorUtil(0x0a080a);
            colorFrom = 0x0a080a;
            colorTo = 0x0a080a;
        } else if (summoned.getType() == ModEntityType.HELLHOUND) {
