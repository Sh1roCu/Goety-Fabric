...
public class MaulingSpell extends SummonSpell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                Summoned summonedentity = new BearServant(ModEntityType.BEAR_SERVANT, worldIn);
                ...
                if (worldIn.isWaterAt(blockPos) || typeStaff(staff, SpellType.ABYSS)) {
                    summonedentity = new Gnasher(ModEntityType.GNASHER, worldIn);
                } else if (typeStaff(staff, SpellType.FROST) || (worldIn.getBiome(blockPos).is(ConventionalBiomeTags.CLIMATE_COLD) && worldIn.getBiome(blockPos).is(ConventionalBiomeTags.IN_OVERWORLD))) {
                    summonedentity = new BearServant(ModEntityType.POLAR_BEAR_SERVANT, worldIn);
                    ...
                    summonedentity = new HoglinServant(ModEntityType.HOGLIN_SERVANT, worldIn);
                }
    ...
    public void summonParticles(ServerLevel worldIn, LivingEntity caster, ItemStack staff, LivingEntity summoned) {
        ...
        if (summoned.getType() == ModEntityType.BEAR_SERVANT) {
            colorUtil = new ColorUtil(0x403b14);
            colorFrom = 0x403b14;
            colorTo = 0x5b4e1d;
        } else if (summoned.getType() == ModEntityType.HOGLIN_SERVANT) {
