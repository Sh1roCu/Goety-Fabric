...
public class SlimySpell extends SummonSpell {
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                SlimeServant slimeServant = new SlimeServant(ModEntityType.SLIME_SERVANT, worldIn);
                ...
                if (specialStaffs(staff)) {
                    if (typeStaff(staff, SpellType.ABYSS)) {
                        slimeServant = new TropicalSlimeServant(ModEntityType.TROPICAL_SLIME_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.NECROMANCY)) {
                        slimeServant = new CryptSlimeServant(ModEntityType.CRYPT_SLIME_SERVANT, worldIn);
                    } else if (typeStaff(staff, SpellType.NETHER)) {
                        slimeServant = new MagmaCubeServant(ModEntityType.MAGMA_CUBE_SERVANT, worldIn);
                    }
                } else {
                ...
                if (slimeServant.getType() != ModEntityType.TROPICAL_SLIME_SERVANT) {
    ...
    public void summonParticles(ServerLevel worldIn, LivingEntity caster, ItemStack staff, LivingEntity summoned) {
        ...
        if (summoned.getType() == ModEntityType.SLIME_SERVANT) {
            colorUtil = new ColorUtil(0x403b14);
            colorFrom = 0x403b14;
            colorTo = 0x5b4e1d;
        } else if (summoned.getType() == ModEntityType.CRYPT_SLIME_SERVANT) {
            colorUtil = new ColorUtil(0x8FE6DF);
            colorFrom = 0x17b0e0;
        } else if (summoned.getType() == ModEntityType.MAGMA_CUBE_SERVANT) {
