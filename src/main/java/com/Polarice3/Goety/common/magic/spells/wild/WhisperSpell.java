    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!isShifting(caster)) {
            ...
            for (int i1 = 0; i1 < i; ++i1) {
                Summoned summonedentity = new Whisperer(ModEntityType.WHISPERER, worldIn);
                ...
                if (worldIn.isWaterAt(blockPos) || this.typeStaff(staff, SpellType.ABYSS)) {
                    summonedentity = new Wavewhisperer(ModEntityType.WAVEWHISPERER, worldIn);
                }
                summonedentity.setTrueOwner(caster);
                summonedentity.moveTo(blockPos, 0.0F, 0.0F);
                if (summonedentity.getType() != ModEntityType.WAVEWHISPERER) {
