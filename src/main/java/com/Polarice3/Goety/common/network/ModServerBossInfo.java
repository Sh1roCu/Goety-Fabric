...
public class ModServerBossInfo extends ServerBossEvent {
    ...
    public void addPlayer(ServerPlayer player) {
        ModNetwork.sendToClient(player, SUpdateBossBar.ID, SUpdateBossBar.encode(this.getId(), this.boss, false));
        if (this.boss.getSensing().hasLineOfSight(player)) {
            super.addPlayer(player);
        } else {
    ...
    public void removePlayer(ServerPlayer player) {
        ...
        ModNetwork.sendToClient(player, SUpdateBossBar.ID, SUpdateBossBar.encode(this.getId(), this.boss, true));
    }
}
