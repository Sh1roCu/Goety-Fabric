...
public class SLightningBoltPacket {
    ...
    public static final ResourceLocation ID = Goety.location("s2c_lightningbolt");

    public static FriendlyByteBuf encode(Vec3 start, Vec3 end, int lifespan) {
        return encode(start, end, new ColorUtil(0x6464dc), lifespan);
    }
    ...
    public static void consume(Minecraft client, ClientPacketListener handler, FriendlyByteBuf buf, PacketSender responseSender) {
        ...
        client.execute(() -> {
            ...
            ColorUtil colorUtil = new ColorUtil(red, green, blue);
            Goety.PROXY.lightningBolt(start, end, colorUtil, lifespan);
        });
    }
}
