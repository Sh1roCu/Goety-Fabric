...
public class SPlayFollowSoundPacket {
    ...
    public static final ResourceLocation ID = Goety.location("s2c_play_follow_sound");

    public static FriendlyByteBuf encode(Entity entity, SoundEvent soundEvent, float volume, float pitch, boolean loop) {
        return encode(entity.getId(), soundEvent, volume, pitch, loop);
    }

    public static FriendlyByteBuf encode(int entityId, SoundEvent soundEvent, float volume, float pitch, boolean loop) {
        ...
        client.execute(() -> {
            ...
            if (level instanceof ClientLevel clientWorld) {
                if (entityId >= 0) {
                    ...
                    if (entity != null) {
                        LoopSoundPlayer.playFollowSound(entity, soundEvent, volume, pitch, loop);
                    }
                }
            }
        });
