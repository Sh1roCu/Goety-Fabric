...
public class SThunderBoltPacket {
    ...
    public static final ResourceLocation ID = Goety.location("s2c_thunderbolt");

    public static FriendlyByteBuf encode(Vec3 start, Vec3 end, int lifespan) {
        return encode(start, end, new ColorUtil(0xb1ebdc), lifespan);
    }
    ...
    public static void consume(Minecraft client, ClientPacketListener handler, FriendlyByteBuf buf, PacketSender responseSender) {
        ...
        client.execute(() -> {
            ...
            Goety.PROXY.thunderBolt(start, end, colorUtil, lifespan);
        });
    }
}
