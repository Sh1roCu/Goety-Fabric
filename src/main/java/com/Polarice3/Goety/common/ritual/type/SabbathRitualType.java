...
public class SabbathRitualType implements IRitualType {
    ...
    public void onFinishRitual(Level world, BlockPos darkAltarPos, DarkAltarBlockEntity tileEntity, Player castingPlayer, ItemStack activationItem) {
        if (tileEntity.getCurrentRitualRecipe().getEntityToSummon() == ModEntityType.SUMMON_APOSTLE) {
            if (world instanceof ServerLevel serverLevel) {
                ModNetwork.sendToALL(serverLevel.getServer(), SPlayWorldSoundPacket.ID, SPlayWorldSoundPacket.encode(darkAltarPos, SoundEvents.AMBIENT_SOUL_SAND_VALLEY_MOOD.value(), 1.0F, 1.0F));
                Warden.applyDarknessAround(serverLevel, Vec3.atCenterOf(darkAltarPos), null, 32);
            }
