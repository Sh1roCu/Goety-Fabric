...
import com.Polarice3.Goety.compat.trinkets.TrinketsLoaded;
...
public class CuriosFinder {
    ...
    public static ItemStack findCurio(LivingEntity livingEntity, Predicate<ItemStack> filter) {
        ...
        if (livingEntity instanceof Player) {
            if (TrinketsLoaded.TRINKETS.isLoaded()) {
                Optional<ItemStack> slotResult = TrinketsApi.getTrinketComponent(livingEntity).
                        flatMap(trinket -> {
                            var results = trinket.getEquipped(filter);
                            if (results.isEmpty()) {
    ...
    public static ItemStack findCurio(LivingEntity livingEntity, Item item) {
        ...
        if (livingEntity instanceof Player) {
            if (TrinketsLoaded.TRINKETS.isLoaded()) {
                Optional<ItemStack> slotResult = TrinketsApi.getTrinketComponent(livingEntity).flatMap(trinket -> {
                    var results = trinket.getEquipped(item);
                    if (results.isEmpty()) {
                        return Optional.empty();
                    } else {
                        return Optional.of(results.get(0).getB());
    ...
    public static ItemStack findCurioInAll(Player playerEntity, Item item) {
        ItemStack foundStack = ItemStack.EMPTY;
        if (TrinketsLoaded.TRINKETS.isLoaded()) {
            Optional<ItemStack> slotResult = TrinketsApi.getTrinketComponent(playerEntity).flatMap(trinket -> {
                var results = trinket.getEquipped(item);
                if (results.isEmpty()) {
                    return Optional.empty();
                } else {
        ...
        if (livingEntity != null) {
            if (TrinketsLoaded.TRINKETS.isLoaded()) {
                return TrinketsApi.getTrinketComponent(livingEntity)
                        .map(trinket -> trinket.getAllEquipped().stream()
                                .map(Tuple::getA)
                                .map(SlotReference::inventory)
                                .noneMatch(inv -> Objects.equals(inv.getSlotType().getGroup(), "head")))
                        .orElse(true);
            }
        }
        return true;
    ...
    public static boolean hasWanting(Entity entity) {
        ...
        if (player != null) {
            ...
            if (ring.getItem() == ModItems.RING_OF_WANT) {
                if (ring.isEnchanted()) {
                    float wanting = EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.WANTING, ring);
                    return wanting > 0;
                }
            }
        }
    ...
    public static boolean hasGeoRobe(LivingEntity livingEntity) {
        return hasCurio(livingEntity, ModItems.GEO_ROBE);
    }

    public static boolean hasAmethystNecklace(LivingEntity livingEntity) {
        return hasCurio(livingEntity, ModItems.AMETHYST_NECKLACE);
    }
    ...
    public static boolean hasWindRobes(LivingEntity livingEntity) {
        return hasCurio(livingEntity, ModItems.WIND_ROBE);
    }
    ...
    public static boolean hasStormRobes(LivingEntity livingEntity) {
        return hasCurio(livingEntity, ModItems.STORM_ROBE);
    }
    ...
    public static ItemStack findRing(Player playerEntity) {
        ItemStack foundStack = ItemStack.EMPTY;
        if (TrinketsLoaded.TRINKETS.isLoaded()) {
            foundStack = findCurio(playerEntity, CuriosFinder::isRing);
        } else {
    ...
    public static ItemStack findBrewInBag(Player player) {
        ...
        if (!findBrewBag(player).isEmpty()) {
            ...
            for (int i = 1; i < brewBagItemHandler.getSlotCount(); ++i) {
    ...
    private static boolean isBrewBag(ItemStack itemStack) {
        return itemStack.getItem() == ModItems.BREW_BAG;
    }

    public static ItemStack findBrewBag(Player playerEntity) {
        ItemStack foundStack = findCurio(playerEntity, CuriosFinder::isBrewBag);
