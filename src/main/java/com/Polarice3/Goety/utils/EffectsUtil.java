...
public class EffectsUtil {
    ...
    public static boolean is(MobEffect effect, TagKey<MobEffect> tagKey) {
        return BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect).is(tagKey);
    }
    ...
    public static List<LivingEntity> addEffectToAllAround(ServerLevel p_216947_, @Nullable Entity p_216948_, Vec3 p_216949_, double p_216950_, MobEffectInstance p_216951_, int p_216952_) {
        ...
        if (illague != null) {
            ...
            if (MobsConfig.IllagueSpread.get()) {
                for (LivingEntity livingEntity : level.getEntitiesOfClass(LivingEntity.class, infected.getBoundingBox().inflate(8.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
                    if (livingEntity.isAlive() && !(livingEntity instanceof PatrollingMonster) && !(livingEntity instanceof RaiderServant) && livingEntity.getMobType() != MobType.UNDEAD && !livingEntity.hasEffect(GoetyEffects.ILLAGUE)) {
            ...
            if (infected.tickCount % 20 == 0) {
                for (int i = 0; i < 8; ++i) {
                    level.sendParticles(ModParticleTypes.PLAGUE_EFFECT, infected.getRandomX(0.5D), infected.getRandomY(), infected.getRandomZ(0.5D), 1, 0.0D, 0.5D, 0.0D, 0);
                }
            }
            ...
            if (k > 0) {
                if ((infected.tickCount % k == 0) && level.getDifficulty() != Difficulty.PEACEFUL) {
                    ...
                    int r4 = r3 + 1;
                    if (r2 == 0) {
                        EffectsUtil.amplifyEffect(infected, GoetyEffects.ILLAGUE, 6000);
                    }
