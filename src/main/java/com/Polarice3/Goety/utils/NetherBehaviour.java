...
public interface NetherBehaviour {
    ...
    static boolean regrowNether(LevelAccessor accessor, BlockPos blockPos, BlockState originalState) {
        ...
        if (originalState.is(BlockTags.IRON_ORES/*Tags.Blocks.ORES_IRON*/) || originalState.is(BlockTags.GOLD_ORES/*Tags.Blocks.ORES_GOLD*/)) {
            BlockState blockstate = Blocks.NETHER_GOLD_ORE.defaultBlockState();
            accessor.setBlock(blockPos, blockstate, 3);
            return true;
        }

        if (originalState.is(BlockTags.COAL_ORES /*Tags.Blocks.ORES_COAL*/) || originalState.is(BlockTags.COPPER_ORES/*Tags.Blocks.ORES_COPPER*/)) {
        ...
        if (accessor instanceof Level level) {
            if (BlockFinder.canBeReplaced(level, blockPos) && !originalState.isAir() && !(originalState.getBlock() instanceof LiquidBlock) && !(originalState.getBlock() instanceof BaseFireBlock)) {
                if ((level.random.nextFloat() <= 0.15F && !(originalState.getBlock().asItem() instanceof ItemNameBlockItem)) || (originalState.getBlock().asItem() instanceof ItemNameBlockItem plantable && !Blocks.NETHERRACK.defaultBlockState().canSurvive(accessor, blockPos.below()))) {
        ...
        if (originalState.is(Blocks.WATER)) {
            ...
            if (!holder.is(BiomeTags.IS_FOREST) && !holder.is(ConventionalBiomeTags.SWAMP) && !holder.is(Biomes.CRIMSON_FOREST)) {
                blockstate = Blocks.NETHERRACK.defaultBlockState();
            }
            accessor.setBlock(blockPos, blockstate, 3);
            accessor.levelEvent(2001, blockPos, Block.getId(blockstate));
            if (above.getBlock().asItem() instanceof ItemNameBlockItem plantable && !blockstate.canSurvive(accessor, blockPos)) {
            ...
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(Blocks.STONE/*Tags.Blocks.STONE*/)) {
            BlockState blockstate = Blocks.BLACKSTONE.defaultBlockState();
            accessor.setBlock(blockPos, blockstate, 3);
            accessor.levelEvent(2001, blockPos, Block.getId(blockstate));
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(Blocks.MUD) || originalState.is(BlockTags.SAND/*Tags.Blocks.SAND*/)) {
            ...
            changeBiome(accessor, blockPos);
            return true;
        } else if (originalState.is(ConventionalBlockTags.SANDSTONE_BLOCKS/*Tags.Blocks.SANDSTONE*/)) {
    ...
    private static BiomeResolver makeResolver(LevelAccessor levelAccessor, MutableInt p_262615_, ChunkAccess p_262698_, BoundingBox p_262622_) {
        return (p_262550_, p_262551_, p_262552_, p_262553_) -> {
            ...
            if (p_262622_.isInside(i, j, k) && !holder.is(BiomeTags.IS_NETHER)) {
                p_262615_.increment();
                if (holder.is(BiomeTags.IS_FOREST) || holder.is(ConventionalBiomeTags.SWAMP)) {
                ...
                if (holder.is(ConventionalBiomeTags.MESA) || holder.is(ConventionalBiomeTags.DESERT)) {
                ...
                if (holder.is(ConventionalBiomeTags.MOUNTAIN)) {
