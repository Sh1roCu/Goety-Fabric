...
public class RenderBlockUtils {
    ...
    public static void renderModelBrightnessColorQuads(PoseStack.Pose matrixEntry, VertexConsumer builder, float red, float green, float blue, float alpha, List<BakedQuad> quads,
        ...
        for (BakedQuad bakedquad : quads) {
            ...
            builder.putBulkData(matrixEntry, bakedquad, new float[]{1.0F, 1.0F, 1.0F, 1.0F}, r, g, b, new int[]{combinedLights, combinedLights, combinedLights, combinedLights}, combinedOverlay, readExistingColor);
        }
    }
    ...
    public static int getColorARGB(FluidVariant fluid) {
        if (fluid.isBlank()) {
            return -1;
        }
        return FluidRenderHandlerRegistry.INSTANCE.get(fluid.getFluid()).getFluidColor(null, null, fluid.getFluid().defaultFluidState());
    }
