...
public class ServantUtil {
    ...
    public static void convertZombies(Entity target, LivingEntity owner, boolean permanent) {
        if (target instanceof Zombie zombieEntity /*&& EventHooks.canLivingConvert(zombieEntity, ModEntityType.ZOMBIE_SERVANT, (timer) -> {
        })*/) {
            EntityType<? extends Mob> entityType = ModEntityType.ZOMBIE_SERVANT;
            if (zombieEntity instanceof ZombieVillager) {
                entityType = ModEntityType.ZOMBIE_VILLAGER_SERVANT;
            } else if (zombieEntity instanceof Husk) {
                entityType = ModEntityType.HUSK_SERVANT;
            } else if (zombieEntity instanceof Drowned) {
                entityType = ModEntityType.DROWNED_SERVANT;
                ...
                entityType = ModEntityType.ZPIGLIN_SERVANT;
            }
    ...
    public static void convertSkeletons(Entity target, LivingEntity owner, boolean wither, boolean permanent) {
        if (target instanceof AbstractSkeleton skeleton /*&& EventHooks.canLivingConvert(skeleton, ModEntityType.SKELETON_SERVANT, (timer) -> {
        })*/) {
            EntityType<? extends Mob> entityType = ModEntityType.SKELETON_SERVANT;
    ...
    public static boolean convertUndead(Mob target, LivingEntity owner, boolean permanent, boolean keepLoot) {
        ...
        if (target instanceof MuckWraith) {
            summoned = target.convertTo(ModEntityType.MUCK_WRAITH_SERVANT, keepLoot);
        } else if (target instanceof BorderWraith) {
            summoned = target.convertTo(ModEntityType.BORDER_WRAITH_SERVANT, keepLoot);
        } else if (target instanceof Wraith) {
            summoned = target.convertTo(ModEntityType.WRAITH_SERVANT, keepLoot);
        } else if (target instanceof Reaper) {
            summoned = target.convertTo(ModEntityType.REAPER_SERVANT, keepLoot);
        } else if (target instanceof Phantom) {
            summoned = target.convertTo(ModEntityType.PHANTOM_SERVANT, keepLoot);
        }
                            ...
                            if (livingOwned.getServer() != null) {
                                ...
                                if (newLevel != null) {
                                    ...
                                    if (optional2.isPresent()) {
                                        ...
                                        Entity entity = FabricDimensions.teleport(livingOwned, newLevel, new ArcaTeleporter(vec32, livingOwned));
                                        if (entity != null) {
