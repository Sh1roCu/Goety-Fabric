...
public class WandUtil {
    ...
    public static ItemStack findFocusOnHand(LivingEntity livingEntity, InteractionHand hand) {
        ItemStack foundStack = ItemStack.EMPTY;
        ItemStack wandOnHand = findWandOnHand(livingEntity, hand);
        if (!wandOnHand.isEmpty()) {
            if (!IWand.getFocus(wandOnHand).isEmpty()) {
                foundStack = IWand.getFocus(wandOnHand);
            }
        }
    ...
    public static ItemStack findFocus(LivingEntity livingEntity) {
        ItemStack foundStack = ItemStack.EMPTY;
        ItemStack found = findWand(livingEntity);
        if (!found.isEmpty()) {
            if (!IWand.getFocus(found).isEmpty()) {
                foundStack = IWand.getFocus(found);
            }
        }
    ...
    public static int getLevels(Enchantment enchantment, LivingEntity livingEntity) {
        if (enchantedFocus(livingEntity)) {
            return EnchantmentHelper.getItemEnchantmentLevel(enchantment, findFocus(livingEntity));
        } else {
            return 0;
        }
    }
    ...
    public static int getPotencyItemLevel(ItemStack itemStack) {
        return EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.POTENCY, itemStack) * SpellConfig.PotencyPower.get();
    }
                ...
                if (target.hurt(damageSource, damage)) {
                    if (prevTarget != target) {
                        ...
                        if (level instanceof ServerLevel serverLevel) {
                            if (small) {
                                ModNetwork.sendToALL(serverLevel.getServer(), SLightningPacket.ID, SLightningPacket.encode(vec3, vec31, colorUtil, 5));
                            } else {
                                ModNetwork.sendToALL(serverLevel.getServer(), SThunderBoltPacket.ID, SThunderBoltPacket.encode(vec3, vec31, colorUtil, 8));
                            }
                        }
                    }
                }
    ...
    public static void spawnIceBouquet(Level level, LivingEntity casterEntity, Vec3 targetPos, double xshift, double zshift, float damage, int duration, boolean isCenter) {
        ...
        IceBouquet iceBouquet = ModEntityType.ICE_BOUQUET.create(level);
        if (iceBouquet != null) {
    ...
    public static void summonTridentStorm(LivingEntity casterEntity, Vec3 targetPos, double xshift, double zshift, int warmUp, int potency) {
        ...
        TridentStorm tridentStorm = ModEntityType.TRIDENT_STORM.create(level);
